...
BlockInfo = NamedTuple(
    "BlockInfo", [("id", int), ("grid_type", GridType), ("label", str), ("ncells", int)]
)
...
class SimInfo:
    ...
    def __init__(self, lmr_cfg):
        ...
        if os.path.exists(fname):
            with open(fname, "r") as fp:
                ...
                for line in content:
                    ...
                    gt = (
                        GridType.STRUCTURED
                        if items[1] == "structured_grid"
                        else GridType.UNSTRUCTURED
                    )
                    bi = BlockInfo(
                        id=int(items[0]),
                        grid_type=gt,
        ...
        fname = os.path.join(
            self.snaps_dir, "fluid" + self.lmr_cfg["metadata-extension"]
        )
        with open(fname, "r") as fp:
        ...
        self.gridarrays = []
        fname = os.path.join(
            os.path.join(self.grid_dir, "grid" + self.lmr_cfg["metadata-extension"])
        )
        with open(fname, "r") as fp:
        ...
        for i in range(grid_metadata["ngridarrays"]):
            ...
            gai = GridArrayInfo(
                tag=mdata["tag"],
                shock_fitting=mdata["shock_fitting"],
                idarray=mdata["idarray"],
                idflatlist=mdata["idflatlist"],
                nib=mdata["nib"],
                njb=mdata["njb"],
                nkb=mdata["nkb"],
                niv=mdata["niv"],
        ...
        for i in range(grid_metadata["ngrids"]):
            ...
            with open(fname, "r") as fp:
                ...
                gt = (
                    GridType.STRUCTURED
                    if mdata["type"] == "structured_grid"
                    else GridType.UNSTRUCTURED
                )
                gi = GridInfo(
                    tag=mdata["tag"],
                    type=gt,
                    field_type=mdata["fieldType"],
    ...
    def load_pvd_into_pyvista(
        self,
        domain_type=DomainType.FLUID,
        snapshot=-1,
        merged=False,
        as_point_data=False,
        make_vtk_files=True,
        ...
        if not os.path.exists(fname):
            if make_vtk_files:
                proc = subprocess.run(
                    ["lmr", "snapshot2vtk", "--all"], capture_output=True
                )
                if not proc.returncode == 0:
                    raise RuntimeError(
                        f"Error when running lmr snapshot2vtk: {proc.stdout}, {proc.stderr}"
                    )
            else:
...
class Snapshot:
    ...
    def __init__(self, sim, snap_name):
        ...
        grid_dir = (
            snap_dir
            if sim.moving_grid
            else os.path.join(sim.snaps_dir, sim.snapshots[0])
        )
        self.grids = sim.read_grids(grid_dir)
        n = len(sim.grids)
        assert n == len(self.grids), (
            f"Expected number of grids ({n}) and actually loaded grids ({len(grids)})."
        )
        ...
        for i in range(n):
            ...
            fname = (
                grid_meta.field_type
                + ("-%04d" % (i))
                + (".bin" if binary_data else ".gz")
            )
            ...
            combined_data = (
                np.frombuffer(f.read(), dtype=float) if binary_data else np.loadtxt(f)
            )
            f.close()
    ...
    def get_slice(self, var, blocks=None, i=None, j=None, k=None):
        ...
        #
        for ib in blocks:
            if self.sim.grids[ib].type == GridType.UNSTRUCTURED:
                continue
            ...
            if dims == 3:
                if i is not None and j is not None:
                    ...
                    values = np.concat(
                        (values, self.fields[ib][var][i, :, k]), axis=None
                    )
                else:
    ...
    def add_vars(self, names):
        ...
        for field in self.fields:
            ...
            if "pitot" in names:
                ...
                t2 = (g + 1) / (2 * g * M * M - (g - 1))
                pitot_p_supersonic = (
                    p * np.pow(t1, (g / (g - 1))) * np.pow(t2, (1 / (g - 1)))
                )
            ...
            if "total_h" in names:
                ...
                for var in emodes_names:
                    e_int += field[var]
                total_h = p / rho + field["e"] + 0.5 * v * v
                if "tke" in current_vars:
                    total_h += field["tke"]
