...
def set_xy_scales():
    ...
    """
    xscale = (cfg["canvas_x_size"] - cfg["canvas_x_offset"] - 10) / (
        cfg["x_max"] - cfg["x_min"]
    )
    yscale = (cfg["canvas_y_size"] - cfg["canvas_y_offset"] - 10) / (
        cfg["y_max"] - cfg["y_min"]
    )
    if cfg["equal_scales"]:
        # Select the smaller scale, such that the full world-range fits.
        minscale = xscale if xscale < yscale else yscale
        cfg["x_scale"] = minscale
        cfg["y_scale"] = minscale
    else:
        cfg["x_scale"] = xscale
        cfg["y_scale"] = yscale
    return
...
def world_y(canvas_y):
    return (cfg["canvas_y_size"] - canvas_y - cfg["canvas_y_offset"]) / cfg[
        "y_scale"
    ] + cfg["y_min"]
...
def init_widgets():
    ...
    status_msg.set("temporary message")
    coord_x = StringVar()
    coord_x.set("0.0")
    coord_y = StringVar()
    coord_y.set("0.0")
    ...
    var_show_node_numbers = StringVar()
    var_show_node_numbers.set(0)
    var_show_char_mesh = StringVar()
    var_show_char_mesh.set(1)
    var_show_streamlines = StringVar()
    var_show_streamlines.set(1)
    var_equal_scales = StringVar()
    var_equal_scales.set(cfg["equal_scales"])
    #
    ...
    c = Canvas(
        cf,
        width=cfg["vport_x_size"],
        height=cfg["vport_y_size"],
    ...
    c.yview_moveto(
        1.0 - cfg["vport_y_size"] / cfg["canvas_x_size"]
    )  # show lower region
    ...
    menu_file.add_command(
        label="Save view as Postscript...", command=savePostscriptFile
    )
    menu_file.add_command(label="Quit", command=quitProgram)
    menu_edit = Menu(menubar)
    menubar.add_cascade(menu=menu_edit, label="Edit")
    menu_edit.add_command(
        label="Clear list of selected nodes", command=clearSelectedNodesList
    )
    menu_edit.add_command(
        label="Delete selected nodes from mesh", command=deleteSelectedNodesFromMesh
    )
    ...
    menu_plot.add_checkbutton(
        label="Equal x,y-scales",
        variable=var_equal_scales,
        onvalue=1,
        offvalue=0,
        command=adjustScalesRefreshDisplay,
    )
    menu_plot.add_checkbutton(
        label="Show node numbers",
        variable=var_show_node_numbers,
        onvalue=1,
        offvalue=0,
        command=refreshDisplay,
    )
    menu_plot.add_checkbutton(
        label="Show char mesh",
        variable=var_show_char_mesh,
        onvalue=1,
        offvalue=0,
        command=refreshDisplay,
    )
...
def quitProgram():
    result = messagebox.askyesno(
        message="Quit program?", icon="question", title="IMOC", type="yesno"
    )
    if result:
        root.destroy()
    return
...
def pickSomething(event):
    ...
    if cfg["pick_action"] == PickAction.node:
        ...
        refreshDisplay()
        showStatusMsg(
            "Zoom to selected range x=[%.3g, %.3g] y=[%.3g, %.3g]"
...
def plotMesh():
    ...
    if show_char_mesh:
        for i in kernel.char_mesh:
            ...
            y = canvas_y(node.y)
            myid = c.create_oval(
                x - r, y - r, x + r, y + r, outline="black", fill="gray", tags="nodes"
            )
            object_id[node.indx] = myid
            node_indx[myid] = node.indx
            if show_node_numbers:
                c.create_text(
                    x, y, text="%d" % node.indx, anchor="center", tags="nodeids"
                )
            if node.cplus_up is not None:
                nb = kernel.nodes[node.cplus_up]
                xb = canvas_x(nb.x)
                yb = canvas_y(nb.y)
                line_seg = c.create_line(
                    x, y, xb, yb, fill="green", width=2, tags="mesh"
                )
                c.lower(line_seg)
            ...
            if node.czero_up is not None:
                ...
                yb = canvas_y(nb.y)
                line_seg = c.create_line(
                    x, y, xb, yb, fill="brown", width=2, tags="stream"
                )
                c.lower(line_seg)
...
def zoomToIncludeAll():
    ...
    if y_max <= y_min:
        y_max = y_min + 1.0
    showStatusMsg(
        "Zoom to include all x=[%.3g, %.3g] y=[%.3g, %.3g]"
