...
def interior(node1, node2, node4=None):
    ...
    while (not converged) and (iteration_count < max_iteration):
        ...
        if kernel.axisymmetric:
            ...
            integralCminus = (
                0.5 * directionCminus * lengthCminus * (axiterm1 + axiterm4)
            )
    ...
    if x4 > x1:
        n4.cminus_up = node1
        n1.cminus_down = node4
    else:
        n4.cminus_down = node1
        n1.cminus_up = node4
...
def insert(node1, node2, node4=None, alpha=0.5):
    ...
    if (n1.cplus_down == node2) and (n2.cplus_up == node1) and (n1.cplus_down != -1):
        ...
        (n1.cminus_down == node2) and (n2.cminus_up == node1) and (n1.cminus_down != -1)
    ):
        n4.cminus_up = node1
        n1.cminus_down = node4
        n2.cminus_up = node4
        n4.cminus_down = node2
    elif (n1.cminus_up == node2) and (n2.cminus_down == node1) and (n1.cminus_up != -1):
        n4.cminus_up = node2
        n2.cminus_down = node4
        n1.cminus_up = node4
        n4.cminus_down = node1
...
def cplus_free(node0, node2, node4=None):
    ...
    while (not converged) and (iteration_count < max_iteration):
        ...
        if abs(denominator) <= 1.0e-12:
            raise RuntimeError(
                "cplus_free: streamline and characteristic are parallel."
            )
...
def cminus_free(node0, node1, node4=None):
    ...
    while (not converged) and (iteration_count < max_iteration):
        ...
        if abs(denominator) <= 1.0e-12:
            raise RuntimeError(
                "cminus_free: streamline and characteristic are parallel."
            )
        lambdaCminus = numerator / denominator
        x4 = x1 + lambdaCminus * cosCminus
        y4 = y1 + lambdaCminus * sinCminus
...
def streamline_intersection_weights(node0, node1, node2):
    ...
    while (not converged) and (iteration_count < max_iteration):
        ...
        if abs(denominator) <= 1.0e-12:
            raise RuntimeError(
                "streamline_intersection_weights: "
...
def march_along_cminus(old_first, new_first, direction):
    ...
    while True:
        ...
        if direction == "up":
            ...
            raise RuntimeError(
                f"Invalid direction for marching along Cminus: {direction}"
            )
        if node2 is None:
            break
    #
...
def march_along_cplus(old_first, new_first, direction):
    ...
    while True:
        ...
        if direction == "up":
            ...
            raise RuntimeError(
                f"Invalid direction for marching along Cplus: {direction}"
            )
        if node1 is None:
            break
    #
...
def get_nodes_along_characteristic(node0, direction):
    ...
    while True:
        ...
        if direction == "cminus_down":
            if node1.cminus_down is None:
                break
            else:
                node_indices.append(node1.cminus_down)
        elif direction == "cminus_up":
            if node1.cminus_up is None:
                break
            else:
                node_indices.append(node1.cminus_up)
        elif direction == "cplus_down":
            if node1.cplus_down is None:
                break
            else:
                node_indices.append(node1.cplus_down)
        elif direction == "cplus_up":
            if node1.cplus_up is None:
                break
            else:
                node_indices.append(node1.cplus_up)
        else:
