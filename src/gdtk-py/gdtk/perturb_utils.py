...
def set_case_running(caseString, caseDict, textString, var):
    ...
    if var not in ["coreRfraction", "CoreRadiusFraction"]:
        ...
        create_file_from_template(
            scriptFileName, "./" + caseString + "/" + scriptFileName, caseDict
        )
        #
        ...
        if "inputFileTemplate" in caseDict:
            ...
            create_file_from_template(
                inputFileName, "./" + caseString + "/" + inputFileName, caseDict
            )
        #
        ...
        if not os.path.exists(caseString):
            ...
            os.chdir(caseString)
            command_text = "nenzfr.py --just-stats --CoreRadiusFraction=" + str(
                caseDict[var]
            )
...
def set_perturbed_values(
    NominalValue, PerturbationMagnitude, TypeOfPerturbation, Levels
):
    """ """
    ...
    if Levels == 3:
        if len(delta) == 1:
            # have [delta]
            values = [NominalValue, NominalValue + delta[0], NominalValue - delta[0]]
        elif len(delta) == 2:
            # have [+delta, -delta]
            values = [NominalValue, NominalValue + delta[0], NominalValue - delta[1]]
    elif Levels == 5:
        if len(delta) == 1:
            # have [delta]
            values = [
...
def write_case_summary(varList, caseDict, caseString, newfile):
    ...
    dataFormat = {
        k: "{0:>" + "{0}".format(max(11, len(k)) + 2) + ".5g}" for k in varList
    }
    titleFormat = {
        k: "{0:{fill}>" + "{0}".format(max(11, len(k)) + 2) + "}" for k in varList
    }
