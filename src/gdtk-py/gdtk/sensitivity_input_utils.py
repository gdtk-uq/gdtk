...
def sensitivity_input_checker(cfg):
    ...
    if "inputVariables" not in cfg:
        print(
            "You have not specified a list of the input variables (that have been perturbed."
        )
        print("    Bailing out.")
        cfg["bad_input"] = True
    ...
    if "inputVariablesUncertainty" not in cfg:
        print(
            "You have not specified a dictionary of the input variables uncertainties."
        )
        print("    Bailing out.")
        cfg["bad_input"] = True
    ...
    for var in perturbedVariables:
        if var not in cfg["inputVariables"]:
            print(
                "'{0}' was perturbed but does not appear in the 'inputVariables' list.".format(
                    var
                )
            )
    ...
    if cfg["typeOfUncertainty"] in ["absolute"]:
        cfg["inputVariablesUncertainty_abs"] = copy.deepcopy(
            cfg["inputVariablesUncertainty"]
        )
        cfg["inputVariablesUncertainty_rel"] = {}
        for var in cfg["inputVariables"]:
            cfg["inputVariablesUncertainty_rel"][var] = (
                cfg["inputVariablesUncertainty_abs"][var] / dictOfCases["case000"][var]
            )
    else:
        cfg["inputVariablesUncertainty_rel"] = copy.deepcopy(
            cfg["inputVariablesUncertainty"]
        )
        cfg["inputVariablesUncertainty_abs"] = {}
        for var in cfg["inputVariables"]:
            cfg["inputVariablesUncertainty_abs"][var] = (
                cfg["inputVariablesUncertainty_rel"][var] * dictOfCases["case000"][var]
            )
