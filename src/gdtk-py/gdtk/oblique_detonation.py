...
class ObliqueDetonation:
    def __init__(
        self, beta, T1, M1, rho1, R=287.0, alpha=1000.0, gamma=6.0 / 5.0, q=300000.0
    ):
        self.beta = beta
        self.T1 = T1
        self.M1 = M1
    ...
    def calculate_X(self, lmbda):
        ...
        a1 = (1.0 / ((self.gamma + 1.0) * self.M1 * sin(self.beta))) * (
            self.a1 / self.alpha
        )
        a2 = 1.0 + self.gamma * MsinBeta2
        a3 = MsinBeta2 - 1.0
        a4 = (
            ((2.0 * MsinBeta2) / (MsinBeta2 - 1) ** 2)
            * ((self.gamma**2 - 1.0) / self.gamma)
    ...
    def calculate_rho(self, lmbda):
        ...
        t5 = ((self.gamma - 1.0) / self.gamma) * (
            2.0 * lmbda * self.q / (self.R * self.T1)
        )
