...
def perturb_input_checker(cfg):
    ...
    if "runCMD" not in cfg:
        ...
        print(
            "    a que. Setting to a default run command ('{0}')".format(cfg["runCMD"])
        )
    #
    ...
    if "typeOfPerturbation" not in cfg and "perturbationMagnitudes" not in cfg:
        ...
        for var in cfg["variablesToPerturb"]:
            if var not in cfg["typeOfPerturbation"]:
                print(
                    "{0} is not present in typeOfPerturbation dictionary.".format(var)
                )
                print("    Bailing out.")
                cfg["bad_input"] = True
            if var not in cfg["perturbationMagnitudes"]:
                print(
                    "{0} is not present in perturbationMagnitudes dictionary.".format(
                        var
                    )
                )
                print("    Bailing out.")
