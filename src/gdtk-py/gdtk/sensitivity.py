...
def run_sensitivity(cfg):
    ...
    nominal = "case000"
    nominalData, allExitVar = read_outfile("./" + nominal + "/" + cfg["resultsFile"])
    ...
    for k in range(len(perturbedVariables)):
        ...
        if cfg["levels"] == 3:
            ...
            sensitivity_abs[var] = {
                k: (
                    highWeighting * (highData[k] - nominalData[k]) / (highX - nominalX)
                    + lowWeighting * (nominalData[k] - lowData[k]) / (nominalX - lowX)
                )
                for k in exitVar
            }
            # print
            ...
            sensitivity_rel[var] = {
                k: sensitivity_abs[var][k] * nominalX / nominalData[k] for k in exitVar
            }
            ...
            tooHighData, dontNeed = read_outfile(
                "./" + tooHigh + "/" + cfg["resultsFile"]
            )
            ...
            tooLowData, dontNeed = read_outfile(
                "./" + tooLow + "/" + cfg["resultsFile"]
            )
            ...
            numer = {
                k: tooHighData[k] / tooHighDeltaX**2
                - tooLowData[k] / tooLowDeltaX**2
                - (highData[k] / highDeltaX**2 - lowData[k] / lowDeltaX**2) * weighting
                - nominalData[k]
                * (
                    1 / tooHighDeltaX**2
                    - 1 / tooLowDeltaX**2
        ...
        uncertainty[var] = {
            k: sensitivity_rel[var][k] * inputUncertainties[var] for k in exitVar
        }
    ...
    write_sensitivity_summary(sensitivity_rel, perturbedVariables, exitVar, "relative")
