...
def write_vtk_structured_grid_file(vtkFile, grid, flow):
    ...
    assert isinstance(grid, StructuredGrid), (
        "grid is expected to be a StructuredGrid object"
    )
    ...
    if grid.dimensions == 2:
        assert grid.nkv == 1 and flow.nkc == 1, (
            "2D grid needs to have niv==1 and nkc==1"
        )
    else:
    ...
    if grid.dimensions == 2:
        fp.write(
            '<StructuredGrid WholeExtent="%d %d %d %d 0 0">\n'
            % (0, grid.niv - 1, 0, grid.njv - 1)
        )
        fp.write(
            '<Piece Extent="%d %d %d %d 0 0">\n' % (0, grid.niv - 1, 0, grid.njv - 1)
        )
    else:
        fp.write(
    ...
    for var in flow.variables:
        fp.write(
            '<DataArray Name="%s" type="Float64" NumberOfComponents="1" format="ascii">\n'
            % (var,)
        )
    ...
    if "vel.x" in flow.variables:
        ...
        if "a" in flow.variables:
            ...
            fp.write(
                '<DataArray Name="Mach" type="Float64" NumberOfComponents="1" format="ascii">\n'
            )
        ...
        fp.write(
            '<DataArray Name="vel.vector" type="Float64" NumberOfComponents="3" format="ascii">\n'
        )
        for velxyz in zip(vxdata, vydata, vzdata):
            fp.write("%g %g %g\n" % velxyz)
        fp.write("</DataArray>\n")
    if "B.x" in flow.variables:
        ...
        fp.write(
            '<DataArray Name="B.vector" type="Float64" NumberOfComponents="3" format="ascii">\n'
        )
        for Bxyz in zip(Bxdata, Bydata, Bzdata):
            fp.write("%g %g %g\n" % Bxyz)
        fp.write("</DataArray>\n")
    fp.write("</CellData>\n")
    #
