...
class Field:
        ...
        if self.new_format:
            ...
            for var in self.variables:
                ...
                if length == 1:
                    ...
                    # Array quantity
                    if self.nkc > 1:
                        dataBlob = dataBlob.reshape(
                            length, self.nkc, self.njc, self.nic
                        )
                    else:
                        dataBlob = dataBlob.reshape(length, self.njc, self.nic)
                    self.data[var] = (
                        dataBlob.transpose()
                    )  # index as [i,j,n] or [i,j,k,n]
            ...
            if dimensions == 2:
                if not self.nkc == 1:
                    raise RuntimeError(
                        f"For a 2D grid, expect nkc==1 but got {self.nkc}"
                    )
                for var in self.variables:
                ...
                for j in range(self.njc):
                    for i in range(self.nic):
                        ...
                        items = text.split(" ")
                        assert nvariables == len(items), (
                            "Wrong number of items on data line."
                        )
    ...
    def __repr__(self):
        str = "Field("
        str += (
            f"ziparchive={self.ziparchive}"
            if self.new_format
            else f"gzfile={self.gzfile}"
        )
