...
class TFIVolume(ParametricVolume):
    ...
    def __init__(
        ...
        if all([iminus, iplus, jminus, jplus, kminus, kplus]):
            ...
            if not (
                approxEqualVectors(self.p000, p000_alt1)
                and approxEqualVectors(self.p000, p000_alt2)
            ):
                raise Exception(
                    "TFIVolume open corner p000={self.p000}, {p000_alt1}, {p000_alt2}"
                )
            p100_alt1 = self.iplus(0.0, 0.0)
            p100_alt2 = self.jminus(1.0, 0.0)
            if not (
                approxEqualVectors(self.p100, p100_alt1)
                and approxEqualVectors(self.p100, p100_alt2)
            ):
                raise Exception(
                    "TFIVolume open corner p100={self.p100}, {p100_alt1}, {p100_alt2}"
                )
            p110_alt1 = self.iplus(1.0, 0.0)
            p110_alt2 = self.jplus(1.0, 0.0)
            if not (
                approxEqualVectors(self.p110, p110_alt1)
                and approxEqualVectors(self.p110, p110_alt2)
            ):
                raise Exception(
                    "TFIVolume open corner p110={self.p110}, {p110_alt1}, {p110_alt2}"
                )
            p010_alt1 = self.iminus(1.0, 0.0)
            p010_alt2 = self.jplus(0.0, 0.0)
            if not (
                approxEqualVectors(self.p010, p010_alt1)
                and approxEqualVectors(self.p010, p010_alt2)
            ):
                raise Exception(
                    "TFIVolume open corner p010={self.p010}, {p010_alt1}, {p010_alt2}"
                )
            p001_alt1 = self.iminus(0.0, 1.0)
            p001_alt2 = self.jminus(0.0, 1.0)
            if not (
                approxEqualVectors(self.p001, p001_alt1)
                and approxEqualVectors(self.p001, p001_alt2)
            ):
                raise Exception(
                    "TFIVolume open corner p001={self.p001}, {p001_alt1}, {p001_alt2}"
                )
            p101_alt1 = self.iplus(0.0, 1.0)
            p101_alt2 = self.jminus(1.0, 1.0)
            if not (
                approxEqualVectors(self.p101, p101_alt1)
                and approxEqualVectors(self.p101, p101_alt2)
            ):
                raise Exception(
                    "TFIVolume open corner p101={self.p101}, {p101_alt1}, {p101_alt2}"
                )
            p111_alt1 = self.iplus(1.0, 1.0)
            p111_alt2 = self.jplus(1.0, 1.0)
            if not (
                approxEqualVectors(self.p111, p111_alt1)
                and approxEqualVectors(self.p111, p111_alt2)
            ):
                raise Exception(
                    "TFIVolume open corner p111={self.p111}, {p111_alt1}, {p111_alt2}"
                )
            p011_alt1 = self.iminus(1.0, 1.0)
            p011_alt2 = self.jplus(0.0, 1.0)
            if not (
                approxEqualVectors(self.p011, p011_alt1)
                and approxEqualVectors(self.p011, p011_alt2)
            ):
                raise Exception(
                    "TFIVolume open corner p011={self.p011}, {p011_alt1}, {p011_alt2}"
                )
            ...
            raise Exception(
                "TFIVolume should be defined by six faces or eight corners."
            )
        return
...
class WireFrameVolume(TFIVolume):
    def __init__(self, c01, c12, c32, c03, c45, c56, c76, c47, c04, c15, c26, c37):
        ...
        super().__init__(
            iminus=west, iplus=east, jminus=south, jplus=north, kminus=bottom, kplus=top
        )
    ...
    def from_extrusion(cls, base_surf, extrude_path):
        ...
        c01 = base_surf.south.clone()
        c01.translate(delta)
        c32 = base_surf.north.clone()
        c32.translate(delta)
        c03 = base_surf.west.clone()
        c03.translate(delta)
        c12 = base_surf.east.clone()
        c12.translate(delta)
        ...
        c15 = extrude_path.clone()
        c15.translate(new_p1 - new_p0)
        c26 = extrude_path.clone()
        c26.translate(new_p2 - new_p0)
        c37 = extrude_path.clone()
        c37.translate(new_p3 - new_p0)
        return WireFrameVolume(
            c01, c12, c32, c03, c45, c56, c76, c47, c04, c15, c26, c37
        )
...
class SweptSurfaceVolume(ParametricVolume):
    ...
    def __init__(self, face0123, edge04):
        if not isinstance(face0123, ParametricSurface):
            raise RuntimeError(
                "SweptSurfaceVolume: face0123 is not a ParametricSurface."
            )
