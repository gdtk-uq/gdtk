...
class StructuredGrid:
    ...
    @classmethod
    def new(cls, **kwargs):
        """Create a new raw instance of the dataclass that bypasses the custom __init__.

        This constructor exists only to allow backwards compatibility with the
        existing __init__ constructor, which does not adhere to Pythonic conventions.
        Arguments are also type-checked, for extra invariance.
        """
        instance = object.__new__(cls)
        field_lookup = {f.name: f for f in fields(cls)}

        for key, value in kwargs.items():
            if key not in field_lookup:
                raise TypeError(f"{cls.__name__}.new() for an unexpected keyword argument '{key}'")
            field = field_lookup[key]
            if not isinstance(value, field.type):
                raise TypeError(f"'{type(value).__name__}' object cannot be interpreted as {field.type.__name__}")
        ...
        for field in fields(cls):
            ...
            if field.default is field.default_factory is MISSING:
                raise TypeError(f"{cls.__name__}.new() missing required argument '{field.name}'")
    ...
    @classmethod
    def from_psurface(cls, psurf, niv, njv, cf_list):
