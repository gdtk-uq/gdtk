...
class XPath(object):
    ...
    def __init__(self, xs=None, fs=None):
        ...
        if xs and fs:
            ...
            for i in range(nseg):
                ys.append(self.fs[i](self.xs[i + 1]))
        else:
    ...
    def bezier2to(self, x1, y1, x2, y2):
        ...
        def xbez(t):
            return x0 * (1 - t) ** 2 + 2 * x1 * (1 - t) * t + x2 * t**2

        def dxdt(t):
            return 2 * ((x1 - x0) * (1 - t) + (x2 - x1) * t)

        def ybez(t):
            return y0 * (1 - t) ** 2 + 2 * y1 * (1 - t) * t + y2 * t**2
...
class XBezier(object):
    ...
    def xbez(self, t):
        return self.bez(self.xbs, t)

    def ybez(self, t):
        return self.bez(self.ybs, t)

    def dxdt(self, t):
        return len(self.dxbs) * self.bez(self.dxbs, t)
