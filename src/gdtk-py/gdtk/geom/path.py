...
class FnPath(Path):
    ...
    def __init__(self, fn):
        ...
        """
        if not callable(fn):
            raise RuntimeError("FnPath: fn is not callable.")
        if not isinstance(fn(0.0), Vector3):
            raise RuntimeError(
                "FnPath: fn did not return a Vector3 instance for t=0.0."
            )
        t_ends = np.array([0.0, 1.0])
        p_ends = fn(t_ends)
        if not isinstance(p_ends, Vector3):
            raise RuntimeError(
                "FnPath: fn did not return a Vector3 instance for array of t values."
            )
        self.fn = fn
        return
...
class Polyline_old(Path):
    ...
    def __call__(self, t):
        ...
        if n == 1:
            return self.segments[0](t)
        i = 0
        while (i < n) and (t > self.t_values[i]):
            i += 1
        i = min(i, n - 1)
        if i == 0:
            t_local = t / self.t_values[0]
        else:
            t_local = (t - self.t_values[i - 1]) / (
...
class ArcLengthParameterizedPath_old(Path):
    ...
    def underlying_t(self, t):
        ...
        i = self._n - 1
        while (L_target < self.arc_lengths[i]) and (i > 0):
            i -= 1
