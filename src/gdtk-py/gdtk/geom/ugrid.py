...
class UnstructuredGrid(object):
    ...
    def __init__(self, sg):
        ...
        if self.dimensions == 2:
            ...
            self.vertices.x = self.vertices.x.swapaxes(
                0, len(sg.vertices.x.shape) - 1
            ).flatten()
            self.vertices.y = self.vertices.y.swapaxes(
                0, len(sg.vertices.x.shape) - 1
            ).flatten()
            self.vertices.z = self.vertices.z.swapaxes(
                0, len(sg.vertices.z.shape) - 1
            ).flatten()
    ...
    def write_to_su2_file(self, file_name):
        ...
        if self.dimensions > 2:
            points.append(col(self.vertices.z))
...
if __name__ == "__main__":
    ...
    njv = 17 + 1
    sgrd = StructuredGrid(
        psurf=patch, niv=niv, njv=njv, tags=["inflow", "outflow", "inflow", "wall"]
    )
