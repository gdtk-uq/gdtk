...
class Vector3(object):
    ...
    def __init__(self, x, y=None, z=None):
        ...
        if isinstance(x, list) or isinstance(x, tuple):
            ...
            self.x = x
            if y is None:
                raise Exception("Expected a number for y coordinate.")
            self.y = y
            if z is None:
                self.z = 0.0 * self.y
            else:
                self.z = z
        args_are_not_real = not (
            isinstance(self.x, numbers.Real)
            and isinstance(self.y, numbers.Real)
            and isinstance(self.z, numbers.Real)
        )
        args_are_not_array = not (
...
def approxEqualVectors(a, b, rel_tol=1.0e-2, abs_tol=1.0e-5):
    ...
    return np.all(
        [
            np.isclose(a.x, b.x, rtol=rel_tol, atol=abs_tol),
