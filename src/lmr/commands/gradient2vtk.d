/**
 * Module for converting Eilmer-native gradient fields to VTK format.
 *
 * Authors: RJG, PJ, KAD, NNG
 * Date: 2024-08-01
 */

module lmr.commands.gradient2vtk;
...
Command gradient2vtkCmd;
string cmdName = "gradient2vtk";

static this()
{
    gradient2vtkCmd.main = &main_;
    gradient2vtkCmd.description = "Convert fields of gradient values to VTK format.";
    gradient2vtkCmd.shortDescription = gradient2vtkCmd.description;
    gradient2vtkCmd.helpMsg = format(
`lmr %s [options]

Convert the gradient values for flow field using one or more snapshots to VTK format.
...
int main_(string[] args)
{
    ...
    variables = readVariablesFromMetadata(lmrCfg.gradientMetadataFile); 
    ...
    File pvdFile = begin_PVD_file(lmrCfg.vtkDir~"/"~lmrCfg.gradientPrefix~".pvd");
    foreach (snap; snaps2process) {
        // We can't process snapshot 0000, no gradient values computed
        ...
        string pvtuFileName = lmrCfg.gradientPrefix ~ "-" ~ snap ~ ".pvtu";
        File pvtuFile = begin_PVTU_file(lmrCfg.vtkDir ~ "/" ~ pvtuFileName, variables);
        foreach (jb; 0 .. nBlocks) {
            readValuesFromFile(data, gradientFilename(to!int(snap), jb), variables, soln.flowBlocks[jb].ncells, fileFmt);
            if (verbosity > 2) writefln("lmr %s: Writing block %d for snapshot %s to disk.", cmdName, jb, snap);
            string vtuFileName = lmrCfg.gradientPrefix ~ "-" ~ format(lmrCfg.blkIdxFmt, jb) ~ "-" ~ snap ~ ".vtu";
