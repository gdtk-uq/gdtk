...
/** This module contains the $(LREF Complex) type, which is used to represent
    ...
    ++ 
    Adapted for use in complexifying the compressible gas flow code, Eilmer.
    Eilmer uses this complex module to perform complex variable differentiation 
    for gradient-based design optimization of high-speed aircraft. 
    Complex variable differentation relies on the sensitivity information being
    stored in the imaginary component. For more information on complex variable
    differentiation, please refer to:
    
    Squire et al., Using Complex Variables to Estimate Derivatives of Real Functions, 
    SIAM review, vol. 40, No. 1, pp. 110-112, March 1998.

    All differences between this modified version and the original module are 
    highlighted throughout the module by the initials KD and an accompanying
    description. For further information on developing a complex number module for 
    complex variable differentation, please refer to:

    Martins, A Coupled-Adjoint Method for High-Fidelity Aero-Structural Optimization,
...
@safe pure nothrow unittest
{
    ...
    auto ctc = c1 * c2;
    assert(approxEqual(cabs(ctc), cabs(c1)*cabs(c2), EPS));
    assert(approxEqual(arg(ctc), arg(c1)+arg(c2), EPS));

    auto cdc = c1 / c2;
    assert(approxEqual(cabs(cdc), cabs(c1)/cabs(c2), EPS));
    ...
    auto cdr = c1 / a;
    assert(approxEqual(cabs(cdr), cabs(c1)/a, EPS));
    assert(approxEqual(arg(cdr), arg(c1), EPS));

    auto cer = c1^^3.0;
    assert(approxEqual(cabs(cer), cabs(c1)^^3, EPS));
    ...
    auto rdc = a / c1;
    assert(approxEqual(cabs(rdc), a/cabs(c1), EPS));
    assert(approxEqual(arg(rdc), -arg(c1), EPS));

    rdc = a / c2;
    assert(approxEqual(cabs(rdc), a/cabs(c2), EPS));
    ...
    auto rec1b = (-1.0) ^^ c1;
    assert(approxEqual(cabs(rec1b), std.math.exp(-PI * c1.im), EPS));
    ...
    auto rec2b = (-1.0) ^^ c2;
    assert(approxEqual(cabs(rec2b), std.math.exp(-2 * PI), EPS));
    ...
    {
        auto cei = c1^^i;
        assert(approxEqual(cabs(cei), cabs(c1)^^i, EPS));
...
// Added several complex-versions of standard math library
// functions. Most of these are taken from the references
// in the header of this file. The original complex module
// definitions for sin, cos, sqrt and abs functions are still
// in place.
// KD 2018
// start ++
@nogc
