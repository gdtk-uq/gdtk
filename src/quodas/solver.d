...
immutable double eps1 = 1.0e-06;              // dRdD finite difference perturbation 
immutable double eps2 = 1.0e-07;              // brute-force finite difference perturbation
...
void main() {
    ...
    else if (simulation_type == "nozzle") final_time = 1.0; 
    string flux_calc = "ausmdv"; // options: van_leer, ausmdv
    size_t interpolation_order = 2;
    ...
    // with shock: b = 0.45, c = 0.6, d = 3.8 
    // without shock: b = 0.347, c = 0.4, d = 4.0
	...
	while (opt_iter < 1e6) {
	    ...
	    double rho = 0.5;
	    double mu = 1.0e-2; // this has a strong effect on convergence
	    double ak = 1.0;
	    double bb = b;
	    double cc = c;
	    double dd = d;
		...
		while (are_design_vars_negative) {
		  ...
		  //writeln("b = ", b, ", num_negative_areas = ", num_negative_areas, ", ",
		  //	  "area0 = ", global_interfaces[0].area, ", area1 = ", global_interfaces[$-1].area, ", ak = ", ak);
...
void ausmdv_flux_calculator(fvinterface f, double gamma) {
    ...
    double K_SWITCH = 10.0; //10.0;
    double C_EFIX = 0.125; //0.125;
    double rL, rR;
    double pL, pR;
    double uL, uR;
    double aL, aR;
