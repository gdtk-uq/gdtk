...
module gas.thermo.therm_perf_gas_mix_thermo;
...
class ThermPerfGasMixtureThermo : ThermodynamicModel {
public:
    ...
    @nogc
    override void update_sound_speed(GasState gs)
    {
        // Reference:
    ...
    @nogc
    override number dudT_const_v(in GasState gs)
    {
        // Using Cv = Cp - R
    ...
    @nogc
    override number dhdT_const_p(in GasState gs)
    {
    ...
    @nogc
    override number dpdrho_const_T(in GasState gs)
    {
        number Rmix = gas_constant(gs);
        return Rmix*gs.T;
    }

    @nogc
    override number gas_constant(in GasState gs)
    {
    ...
    @nogc
    override number internal_energy(in GasState gs)
    {
...
private:
    ...
    {
        return dhdT_const_p(gs)/dudT_const_v(gs);
    }
...
version(therm_perf_gas_mix_thermo_test) {
    int main() {
        ...
        auto tm = new ThermPerfGasMixtureThermo(L, speciesNames);
        lua_close(L);
        ...
        s = 11000.0;
        number h = 17.0e6;
        tm.update_thermo_from_hs(gs, h, s);
        ...
        version(complex_numbers) {
            ...
            assert(approxEqual(myCv, tm.dudT_const_v(gs).re), failedUnitTest());
        }
