 ...
 *
 * Notes: NNG has enabled this code to be used with complex numbers.
 *        User beware however that the quality of the derivatives
 *        you get when using this code in the steady-state solver
 *        is unknown. (15/05/26)
 */
...
unittest {
    ...
    assert(isClose(e_given, Q.u.re, 1.0e-4));
    assert(isClose(rho_given, Q.rho.re, 1.0e-4));
    assert(isClose(a_given, Q.a.re, 1.0e-4));
    assert(isClose(Cp_given, Cp.re, 1.0e-3));
    assert(isClose(h_given, h.re, 1.0e-4));
    assert(isClose(mu_given, Q.mu.re, 1.0e-4));
    assert(isClose(k_given, Q.k.re, 1.0e-4));
    assert(isClose(s_given, s.re, 1.0e-4));
    assert(isClose(R_given, R.re, 1.0e-4));
    ...
    assert(gm.n_modes == 0);
    assert(gm.n_species == 1);
    assert(isClose(e_given, Q.u.re, 1.0e-3));
    assert(isClose(rho_given, Q.rho.re, 1.0e-3));
    assert(isClose(a_given, Q.a.re, 1.0e-3));
    assert(isClose(Cp_given, Cp.re, 1.0e-3));
    assert(isClose(h_given, h.re, 1.0e-3));
    assert(isClose(mu_given, Q.mu.re, 1.0e-2));
    assert(isClose(k_given, Q.k.re, 1.0e-3));
    assert(isClose(s_given, s.re, 1.0e-3));
    assert(isClose(R_given, R.re, 1.0e-3));
}
