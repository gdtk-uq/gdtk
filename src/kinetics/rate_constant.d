...
public:
    ...
    this(lua_State* L, Tuple!(int, double)[] efficiencies, GasModel gmodel) {
        ...
        if (!lua_isnil(L, -1)) {
            ...
            lua_pop(L, 1);
            lua_getfield(L, -1, "a");
            _a = luaL_checknumber(L, -1);
            lua_pop(L, 1);
            lua_getfield(L, -1, "T3");
            _T3 = luaL_checknumber(L, -1);
            lua_pop(L, 1);
            lua_getfield(L, -1, "T1");
            if (!lua_isnumber(L, -1)) {
                _T1_supplied = false;
...
public:
    ...
    this(lua_State* L, Tuple!(int, double)[] efficiencies, GasModel gmodel) {
        ...
        lua_pop(L, 1);
        lua_getfield(L, -1, "b");
        _b = luaL_checknumber(L, -1);
        lua_pop(L, 1);
        lua_getfield(L, -1, "c");
        _c = luaL_checknumber(L, -1);
        lua_pop(L, 1);
...
unittest {
    ...
    auto rc = new ArrheniusRateConstant(1.94e14 * 1e-6, 0.0, 20_620.0, -1);
    auto gd = GasState(1, 1);
    gd.T = 700.0;
    writeln("rc=", rc.eval(gd)); // rc=3.12412e-05
    assert(isClose(3.10850956e-5, rc.eval(gd), 1.0e-2));
    ...
    gd.T = 4000.0;
    writeln("rc2=", rc2.eval(gd)); // rc2=0.00159439
    assert(isClose(0.00159439, rc2.eval(gd), 1.0e-5));
}
