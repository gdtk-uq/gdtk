...
class VibSpecificCORelaxation : ThermochemicalReactor {
    ...
    this(string fname, GasModel gmodel) {
        ...
        if (!gm) {
            throw new Error("Oops, wrong gas model; should have been VibSpecificCO.");
        }
        ...
        Pvvm1.length = L;
        Pvvm1_wm1w.length = L;
        foreach (ref P; Pvvm1_wm1w)
            P.length = L;
        for (int v = 0; v < gm.n_vibe_states; v++)
            E[v] = vibe_energy(v);
    }
...
protected:
    ...
    @nogc const double vibe_energy(int v) // Returns the quantum level energy for species i.
    {
...
final class VibSpecificCOMixtureRelaxation : VibSpecificCORelaxation {
    ...
    this(string chem_file_name, string kinetics_file_name, GasModel gmodel) {
        ...
        if (!gm) {
            throw new Error("Oops, wrong gas model; should have been VibSpecificCOMixture.");
        }
        ...
        while (lua_next(L, -2) != 0) { // -1 is the dummy key, -2 is the mechanism table
            ...
            if (rateModel != "Landau-Teller")
                throw new Error("Supplied Energy Exchange Mechanism is of the wrong type!");
...
unittest {
    ...
    */
    ...
    foreach (v; 0 .. gm.n_vibe_states) {
        Q.massf[v] = gm.boltzmann_eq_population_fraction(v, Tvib);
    }
    ...
    foreach (i; 0 .. nrows) {
        ...
        assert(isClose(Pvvm1_times_c, Pvvm1_times_c_target, 5.0e-3));
        assert(isClose(Pvvm1_wwp1_times_c, Pvvm1_wwp1_times_c_target, 5.0e-3));
    }
...
unittest {
    ...
    Qmix.T = 175.0; // K
    // Set up the species mass fractions assuming vibrational equilibrium.
    foreach (v; 0 .. gmmix.n_vibe_states) {
        Qmix.massf[v] = gmmix.boltzmann_eq_population_fraction(v, Tvib);
    }
    ...
    foreach (v; 0 .. gm.n_vibe_states) {
        ...
        assert(isClose(source[v], sourcemix[v], 1.0e-6));
    }
    assert(isClose(sourcemix[gmmix.n_species - 1], 0.0, 1.0e-12));
    ...
    Qn2.T = 175.0; // K
    // Set up the species mass fractions assuming vibrational equilibrium.
    foreach (v; 0 .. gmn2.n_vibe_states) {
        Qn2.massf[v] = 0.5 * gmn2.boltzmann_eq_population_fraction(v, Tvib);
    }
