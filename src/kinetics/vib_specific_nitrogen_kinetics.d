...
final class VibSpecificNitrogenRelaxation : ThermochemicalReactor {
                         ...
                         ref number[] params)
    {
        ...
        foreach(imode; 1 .. N_VIB_LEVELS-1) {
            number rho_dot = (Q.rho*Q.rho/_M_N2) *
                (B_coeff(imode,Q.T)*Q.massf[imode-1] +
                 (-F_coeff(imode, Q.T) - B_coeff(imode+1, Q.T))*Q.massf[imode] 
                 + F_coeff(imode+1, Q.T)*Q.massf[imode+1]);
            number rho_i = (rho_dot * dt) + Q.massf[imode]*Q.rho;
            Q.massf[imode] = rho_i / Q.rho;
        }
        //Hard coding the replenishing equation (ground state)
        number rho_dot_0 = (Q.rho*Q.rho/_M_N2) *
            ((-B_coeff(1, Q.T)*Q.massf[0]) + (F_coeff(1, Q.T)*Q.massf[1])); 
        number rho_0 = (rho_dot_0 * dt) + Q.massf[0]*Q.rho;
        Q.massf[0] = rho_0 / Q.rho;
        
        //Hard coding the depleting equation (last quantum level)
        number rho_dot_l = (Q.rho*Q.rho/_M_N2) *
            (B_coeff(N_VIB_LEVELS-1, Q.T)*Q.massf[N_VIB_LEVELS-1-1] +
             (-F_coeff(N_VIB_LEVELS-1, Q.T))*Q.massf[N_VIB_LEVELS-1]); 
                                             
        number rho_l = (rho_dot_l * dt) + Q.massf[N_VIB_LEVELS-1]*Q.rho;
...
private:
    ...
    double vib_energy(int i) const 
    {
     ...
     number F_coeff(int i, number Temp) const
     {
        double I = i + 1;
        number ft = 1e-6 * Avogadro_number * exp(-3.24093 - (140.69597/Temp^^0.2));
        number delta_t = 0.26679 - (6.99237e-5 * Temp) + (4.70073e-9 * Temp^^2);
        number f = (I-1) * ft * exp((I-2)*delta_t);
        return f;     
     }   
     
     number B_coeff(int i, number Temp) const
     {
        number B = F_coeff(i,Temp) * exp(-(vib_energy(i) - vib_energy(i-1)) / (kB * Temp));
