...
function transformRelaxationTime(rt, p, q)
   t = {}
   t.model = rt[1]
...
function relaxationTimeToLuaStr(rt)
   ...
   if rt.model == "Millikan-White" then
      str = string.format("{model='Millikan-White', a=%9.6e, b=%9.6e}", rt.a, rt.b)
   else
...
function expandPColliders(t)
   ps = {}
   unique_ps = {}
   for i,p in ipairs(t) do
      if p == "*molcs" then
         for _,m in molecules do
...
function addUserMechToTable(m, mechanisms)
   t = parseMechString(m[1])
   ps = expandPColliders(t[1])
   qs = expandQColliders(t[2])
   ...
   for _,p in ipairs(ps) do
      ...
      for __,q in ipairs(qs) do
         ...
         mechanisms[p][q].rt = transformRelaxationTime(m.relaxation_time, p, q)
      end
   end
