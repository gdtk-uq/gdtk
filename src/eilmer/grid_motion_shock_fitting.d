...
version(mpi_parallel) {
    ...
    void compute_vtx_velocities_for_sf(FBArray fba)
    {
        ...
        foreach (jb; 0 .. fba.njb) {
            int j0 = 0; foreach(j; 0 .. jb) { j0 += fba.njcs[j]; }
            foreach (kb; 0 .. fba.nkb) {
                int k0 = 0; foreach(k; 0 .. kb) { k0 += fba.nkcs[k]; }
                blkId = fba.blockArray[0][jb][kb];
                blk = cast(SFluidBlock) globalBlocks[blkId];
                foreach (k; 0 .. blk.nkc) {
                    foreach (j; 0 .. blk.njc) {
                        // [TODO] PJ 2021-02-16 This is first-order for R0.  Linear interpolation may be better.
        ...
        //
        // Compute the shock-boundary vertex velocities
        // with an upwind-weighting of the face-centre velocities,
        // as described by Ian.
        // Note that the vertex velocities are aligned with the rails.
        //
        ...
        foreach (jb; 0 .. fba.njb) {
            ...
            foreach (kb; 0 .. fba.nkb) {
                ...
                foreach (ib; 0 .. fba.nib) {
                    ...
                    blk = cast(SFluidBlock) globalBlocks[blkId];
                    int i0 = 0; foreach(i; 0 .. ib) { i0 += fba.nics[i]; }
                    foreach (k; 0 .. blk.nkv) {
                        foreach (j; 0 .. blk.njv) {
...
number wave_speed(const(FlowState) L0, const(FlowState) R0, const(Vector3) n)
{
    ...
    // The face normal, n, is pointing into the domain.
    //
...
number Mach_weighting(number M)
{
    ...
    return (M <= 1.0) ? (0.125*((M+1.0)^^2 + (M+1.0)*fabs(M+1.0))) : M;
}
