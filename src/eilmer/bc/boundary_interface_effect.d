...
BoundaryInterfaceEffect make_BIE_from_json(JSONValue jsonData, int blk_id, int boundary)
{
	...
	break;
	case "energy_balance":
	double emissivity = getJSONdouble(jsonData, "emissivity", 300.0);
	newBIE = new BIE_EnergyBalance(blk_id, boundary, emissivity);
	break;
...
class BIE_EnergyBalance : BoundaryInterfaceEffect {
public:
    double emissivity;
    double SB_sigma_SI = 5.670373e-8; //  Stefan-Boltzmann constant W/(m^2 K^4)

    this(int id, int boundary, double emissivity)
    {
	super(id, boundary, "EnergyBalance");
	this.emissivity = emissivity;
    }

    override string toString() const 
    {
		return "EnergyBalance(emissivity=" ~ to!string(emissivity) ~ ")";
    }

    override void apply_unstructured_grid(double t, int gtl, int ftl)
    {
		throw new FlowSolverException("BIE_EnergyBalance bc not implemented for unstructured grids.");
    }
	...
	//printf("Twall= %.2f K\n", Twall);
	//printf("q_rad = %.2f  W/m^2\n", q_rad);
	//printf("q_cond %.2f  W/m^2\n", q_cond);
	//printf("q_total %.2f W/m^2\n", q_total);


	return;
    } // end solve_for_wall_temperature()
} // end class BIE_EnergyBalance
