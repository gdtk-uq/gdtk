...
// The mpi version of the program that will assign a normalised radial distance to each vertex.
// Could be more efficient, but only runs once so not really an issue.
...
version(mpi_parallel) {
    void assign_radial_dist_mpi(SFluidBlock blk) {
        ...
        // Each block needs to go do its own bit of calculation first
        // so we'll post a non-blocking receive for 2 things-
        // the total radial distance up to this block, and the number of blocks
        ...
        if (blk.bc[Face.east].type == "exchange_over_full_face") {
            ...
            if (find(GlobalConfig.localFluidBlockIds, east_neighbour).empty) {
                ...
                int mpi_partner_recv_tag = make_mpi_tag(east_neighbour, 4, 1);
                east_neighbour_rank = GlobalConfig.mpi_rank_for_block[east_neighbour];
                int ne = to!int(total_dist.length);
                MPI_Irecv(total_dist.ptr, ne, MPI_DOUBLE, east_neighbour_rank, mpi_dist_recv_tag,
                          MPI_COMM_WORLD, &MPI_incoming_dist_request);
                MPI_Irecv(&no_partners, 1, MPI_INT, east_neighbour_rank, mpi_partner_recv_tag,
        ...
        // Now we know how big the chained_to array is, call for it.
        // We can't request it until we know how many elements it should have
        int[] chained_to = new int[no_partners];
        if (requested_data) {
            int mpi_chained_recv_tag = make_mpi_tag(east_neighbour, 4, 2);
            MPI_Recv(chained_to.ptr, no_partners, MPI_INT, east_neighbour_rank, mpi_chained_recv_tag,
                     MPI_COMM_WORLD, &MPI_incoming_chained_status);
        }
        ...
        if (blk.bc[Face.west].type == "exchange_over_full_face") {
            ...
            if (find(GlobalConfig.localFluidBlockIds, west_neighbour).empty) {
                ...
                int mpi_partner_send_tag = make_mpi_tag(blk.id, 4, 1);
        ...
        and give every block a copy of those it is chained to.
        ++/
        if (sent_data) {
            // If I sent data on the way out, I must receive data on the way back
            int ne = to!int(total_dist_send.length);
            int mpi_dist_recv_tag = make_mpi_tag(west_neighbour, 3, 3);
            int mpi_partner_recv_tag = make_mpi_tag(west_neighbour, 3, 4);
            MPI_Irecv(total_dist.ptr, ne, MPI_DOUBLE, west_neighbour_rank, mpi_dist_recv_tag,
                      MPI_COMM_WORLD, &MPI_incoming_dist_request);
            MPI_Irecv(&no_partners, 1, MPI_INT, west_neighbour_rank, mpi_partner_recv_tag,
                      MPI_COMM_WORLD, &MPI_incoming_partner_request);
            MPI_Wait(&MPI_incoming_dist_request, &MPI_incoming_dist_status);
        } else {
        ...
        if (sent_data) {
            ...
            MPI_Recv(chained_to.ptr, no_partners, MPI_INT, west_neighbour_rank, mpi_chained_recv_tag,
                     MPI_COMM_WORLD, &MPI_incoming_chained_status);
        }
        if (requested_data) {
            // If I requested data on the way out, I must send data on the way back
            int ne = to!int(total_dist_send.length);
            int mpi_dist_send_tag = make_mpi_tag(blk.id, 3, 3);
            int mpi_partner_send_tag = make_mpi_tag(blk.id, 3, 4);
            int mpi_chained_send_tag = make_mpi_tag(blk.id, 3, 5);
...
Vector3[] shock_fitting_vertex_velocities(SFluidBlock blk) {
    /++
    For a given block, loop through cell vertices and update the vertex velocities.
    The boundary vertex velocities are set via the methodology laid out
    ...
    // inflow is a reference to a duplicated flow state,
        ...
        for ( size_t j = blk.jmin; j <= blk.jmax+1; ++j ) {
            ...
            if (shock_detect < SHOCK_DETECT_THRESHOLD) { 
                ...
                for (i = 0; i < numcells; ++i) {
                    ...
                    p_east = scalar_reconstruction(cell[i].fs.gas.p, cell_R1[i].fs.gas.p, cell_R2[i].fs.gas.p,
                                                   cell[i].iLength, cell_R1[i].iLength, cell_R2[i].iLength, inflow.gas.p);
                    u_east.refx = scalar_reconstruction(cell[i].fs.vel.x,  cell_R1[i].fs.vel.x, cell_R2[i].fs.vel.x,
                                                        cell[i].iLength, cell_R1[i].iLength, cell_R2[i].iLength, inflow.vel.x);
                    u_east.refy = scalar_reconstruction(cell[i].fs.vel.y, cell_R1[i].fs.vel.y, cell_R2[i].fs.vel.y,
                                                        cell[i].iLength, cell_R1[i].iLength,  cell_R2[i].iLength, inflow.vel.y);
                    u_east.refz = scalar_reconstruction(cell[i].fs.vel.z, cell_R1[i].fs.vel.z, cell_R2[i].fs.vel.z,
                                                        cell[i].iLength, cell_R1[i].iLength, cell_R2[i].iLength, inflow.vel.z);
                    //Find the shock speeds ws1 and ws2 as outlined on page 73 of Ian Johnston's thesis
                    Vector3 ns, t_dim, t_unit;
                    number sign_ws2, sqrt_ws2, M;
                    number mixing_param = 0.5;
                    number ws1, ws2;
                    ...
                    // Create a unit vector which points from the centre of the interface
                    // to the vertex we are interested in
                    t_dim = vtx.pos[0] - cell[i].iface[Face.west].pos;
                    t_unit = (t_dim) / geom.abs(t_dim);
                    // Find the post shock Mach number in the direction of this pointing vector
                    // for use in the weighting function
            ...
            /++ Fill in the correct slot in the master array of vertex velocities.
             Do k-kmin to find how many rows have passed, multiply by jmax-jmin+1
...
} // end assign_slave_velocities()

Vector3 correct_direction(Vector3[] pos) {
    // Sets the direction for vertex movement
    // As Ian Johnston recommends in his thesis, we force the vertices to move along "rails"
    // which are the radial lines originating from the EAST boundary spanning to the west
    Vector3 unit_d;
    ...
    if (blk.bc[Face.south].type == "exchange_over_full_face") {
        ...
        version(mpi_parallel) {
            if (find(GlobalConfig.localFluidBlockIds, neighbour).empty) {
                ...
                MPI_Irecv(incoming_cells.ptr, ne, MPI_DOUBLE, neighbour_rank, mpi_recv_tag,
    ...
    if (blk.bc[Face.north].type == "exchange_over_full_face") {
        ...
        version(mpi_parallel) {
            if (find(GlobalConfig.localFluidBlockIds, neighbour).empty) {
                ...
                i = 0;
                for (size_t k = blk.kmin; k <= krangemax; k++) {
                    blk.get_cell(blk.imin, blk.jmax+1, k).iface[Face.west].n.set(incoming_cells[i],
                                                                                 incoming_cells[i+1],
                                                                                 incoming_cells[i+2]);
                    blk.get_cell(blk.imin, blk.jmax+1, k).iface[Face.west].pos.set(incoming_cells[i+3],
                                                                                   incoming_cells[i+4],
                                                                                   incoming_cells[i+5]);
                    i += 6;
                }
    ...
    if (blk.bc[Face.west].type == "exchange_over_full_face") {
        ...
        version(mpi_parallel) {
            if (find(GlobalConfig.localFluidBlockIds, neighbour).empty) {
                ...
                for (size_t k = blk.kmin; k <= krange; k++) {
                    for (size_t j = blk.jmin; j <= blk.jmax+1; j++) {
                        blk.get_vtx(blk.imin-1, j, k).pos[0].set(incoming_cells[i++],
                                                                 incoming_cells[i++],
                                                                 incoming_cells[i++]);
                    }
                }
        ...
        // There's no real vertex here-
        // we'll just extrapolate it out based on the last 2 vertex locations.
        for (size_t k = krange; k <= krange; k++) {
            for (size_t j = blk.jmin; j <= blk.jmax+1; j++) {
                blk.get_vtx(blk.imin-1, j, k).pos[0] = blk.get_vtx(blk.imin, j, k).pos[0] +
                    (blk.get_vtx(blk.imin, j, k).pos[0] - blk.get_vtx(blk.imin+1, j, k).pos[0]);
            }
        }
    }
    ...
    if (blk.bc[Face.east].type == "exchange_over_full_face") {
        ...
        version(mpi_parallel) {
            if (find(GlobalConfig.localFluidBlockIds, neighbour).empty) {
                ...
                for (size_t k = blk.kmin; k <= krange; k++) {
                    for (size_t j = blk.jmin; j <= blk.jmax+1; j++) {
                        blk.get_vtx(blk.imax+2, j, k).pos[0].set(incoming_cells[i++],
                                                                 incoming_cells[i++],
                                                                 incoming_cells[i++]);
                    }
                }
        ...
        for (size_t k = blk.kmin; k <= krange; k++) {
            for (size_t j = blk.jmin; j <= blk.jmax+1; j++) {
                blk.get_vtx(blk.imax+2, j, k).pos[0] = blk.get_vtx(blk.imax+1, j, k).pos[0] +
                    (blk.get_vtx(blk.imax+1, j, k).pos[0] - blk.get_vtx(blk.imax, j, k).pos[0]);
            }
        }
    }
...
// Next two are actually referred to by simcore, inside set_grid_velocities
// for the case GridMotion.shock_fitting.

number[] unpack_vertex_velocities(Vector3[] vertex_velocities_as_vector) {
    // Unpack the velocities from Vector3s to an array of numbers
    size_t ne = vertex_velocities_as_vector.length * 3;
...
Vector3[] pack_vertex_velocities(number[] vertex_velocities_as_elements) {
    ...
    size_t ne = vertex_velocities_as_elements.length / 3;
    Vector3[] packed_vertex_velocities;
    packed_vertex_velocities.length = ne;
    size_t i = 0;
    foreach (j; 0 .. ne) {
        packed_vertex_velocities[j].set(vertex_velocities_as_elements[i++],
                                        vertex_velocities_as_elements[i++],
                                        vertex_velocities_as_elements[i++]);
    }
...
// Reconstruct the quantities for shock detection and movement, just to the right of the shock face.
...
number scalar_reconstruction(number x1, number x2, number x3, number h1, number h2, number h3, number g1) {
    ...
    if (johnston_reconstruction) {
        // The more accurate reconstruction scheme from Johnston,
        // which is more accurate for the schemes where there is no discontinuity
        // at the boundary due to the selective upwinding bias.
        number kappa = 0.5;
        ...
        number s_f2 = (2 * delta_plus_f2 * delta_minus_f2 + eps) /
            (delta_plus_f2 * delta_plus_f2 + delta_minus_f2 * delta_minus_f2 + eps);
        number delta_1 = (s_f2 / 2.0) * ((1 - s_f2 * kappa) * delta_circleplus_f2 +
                                         (1 + s_f2 * kappa) * delta_circleminus_f2);
