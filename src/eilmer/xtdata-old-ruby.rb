...
#     2019-10-24 Ruby 1.8.x flavour of Open3 for old installations.
#
require 'getoptlong'
require 'open3'
require 'fileutils'
...
tindx_list.each do |tindx|
  ...
  Open3.popen3(*cmd.split) do | stdin, stdout, stderr |
    o = stdout.readlines
    puts "tindx=#{tindx} time=#{time_list[tindx]} data-line-count=#{o.count}"
    found_labels = false
    o.each do |line|
