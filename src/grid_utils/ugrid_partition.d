...
void printHelp()
{
    ...
    writeln(" > ugrid_partition grid-file.su2 mapped-cells.txt --nblks=N --dim=2|3 [--reorder=false|true] [--ptype=kway|rb] [--objtype=cut|vol] [--ncuts=N] [--niter=N] [--seed=N] [--contig=true|false] [--minconn=true|false] [--ufactor=N] [--protect-tags=tag1,tag2] [--protect-layers=N] [--protect-normal-weight=W] [--protect-tangential-weight=W]");
    writeln("");
    writeln(" Legacy positional format, still supported:");
    writeln(" > ugrid_partition grid-file.su2 mapped-cells.txt nPartitions nDim [reorder] [--ptype=kway|rb] [--objtype=cut|vol] [--ncuts=N] [--niter=N] [--seed=N] [--contig=true|false] [--minconn=true|false] [--ufactor=N] [--protect-tags=tag1,tag2] [--protect-layers=N] [--protect-normal-weight=W] [--protect-tangential-weight=W]");
    writeln("");
    writeln("   where:");
    ...
    writeln("   --protect-tags         : optional comma-separated SU2 boundary tags to protect from partition cuts.");
    writeln("   --protect-layers       : optional number of cell-neighbour layers from protected tags to weight.");
    writeln("   --protect-normal-weight : optional METIS edge weight for wall-normal/layer-to-layer edges inside protected region; default 10000.");
    writeln("   --protect-tangential-weight : optional METIS edge weight for same-layer/tangential edges inside protected region; default 1.");
    writeln("");
...
void cleanDir(string dualFile, string partitionFile, string metisFormatFile, string metisPartitionInputFile) {
    // Removes temporary files generated for Metis partitioning.

    if (exists(dualFile)) remove(dualFile);
    if (exists(partitionFile)) remove(partitionFile);
    if (exists(metisFormatFile)) remove(metisFormatFile);

    if (metisPartitionInputFile != dualFile && exists(metisPartitionInputFile)) {
        remove(metisPartitionInputFile);
    }
...
struct BoundaryProtectionOptions {
    string[] tags;
    int layers = 0;
    int normalWeight = 10000;
    int tangentialWeight = 1;
}
...
struct CommandLineOptions {
    ...
    BoundaryProtectionOptions protection;
}
...
void parseOptionalArgument(ref CommandLineOptions options, string arg) {
    ...
    case "ufactor":
        ...
        break;
    case "protect-tags":
        options.protection.tags.length = 0;
        foreach (tag; value.split(",")) {
            string cleanTag = tag.strip();
            if (cleanTag.length > 0) options.protection.tags ~= cleanTag;
        }
        break;
    case "protect-layers":
        options.protection.layers = to!int(value);
        break;
    case "protect-normal-weight":
        options.protection.normalWeight = to!int(value);
        break;
    case "protect-tangential-weight":
        options.protection.tangentialWeight = to!int(value);
        break;
    default:
...
void loadCellAdjacencyFromDualGraph(string dualFile, ref Domain grid) {
    // Attach neighbour cells using cell interconnectivity from a METIS dual graph file.
    auto f = File(dualFile, "r");
    f.readln(); // skip header
    foreach(ref cell; grid.cells) {
        cell.cell_cloud_ids.length = 0;
        auto lineContent = f.readln().strip();
        auto tokens = lineContent.split();
        foreach(tok; tokens) {
            if (tok.length > 0) cell.cell_cloud_ids ~= to!size_t(tok) - 1;
        }
    }
}

bool[] findProtectedBoundaryAdjacentCells(ref Domain grid, string[] protectedTags) {
    // Return true for cells adjacent to any protected SU2 boundary tag.
    bool[] isBoundaryAdjacentCell;
    isBoundaryAdjacentCell.length = grid.cells.length;

    foreach(ref cell; grid.cells) {
        if (cell.face_node_ids.length < 1) cell.construct_faces(grid.dimensions);

        foreach(faceTag; cell.face_tag_list) {
            foreach(ref bc; grid.boundary) {
                if (!protectedTags.canFind(bc.tag)) continue;
                if (binarySearch(faceTag, bc.face_tag_list)) {
                    isBoundaryAdjacentCell[cell.id] = true;
                    break;
                }
            }
            if (isBoundaryAdjacentCell[cell.id]) break;
        }
    }
    return isBoundaryAdjacentCell;
}

int[] computeBoundaryLayerIndex(ref Domain grid, bool[] isBoundaryAdjacentCell, int maxLayers) {
    // Label cells by graph distance, measured in neighbour layers from the protected boundary.
    // Boundary-adjacent cells start at layer 0, then the distance increases outward.
    int[] boundaryLayerIndex;
    boundaryLayerIndex.length = grid.cells.length;
    boundaryLayerIndex[] = int.max;

    size_t[] cellsToVisit;
    foreach(i, isBoundaryAdjacent; isBoundaryAdjacentCell) {
        if (isBoundaryAdjacent) {
            boundaryLayerIndex[i] = 0;
            cellsToVisit ~= i;
        }
    }

    size_t nextCellToVisit = 0;
    while (nextCellToVisit < cellsToVisit.length) {
        size_t cid = cellsToVisit[nextCellToVisit++];
        if (boundaryLayerIndex[cid] >= maxLayers) continue;

        foreach(ncid; grid.cells[cid].cell_cloud_ids) {
            if (boundaryLayerIndex[ncid] == int.max) {
                boundaryLayerIndex[ncid] = boundaryLayerIndex[cid] + 1;
                cellsToVisit ~= ncid;
            }
        }
    }
    return boundaryLayerIndex;
}

string writeBoundaryWeightedDualGraph(string originalDualFile,
                                      ref Domain grid,
                                      int[] boundaryLayerIndex,
                                      int layers,
                                      int normalWeight,
                                      int tangentialWeight) {
    // Rewrite METIS dual graph with integer edge weights.
    // METIS fmt=001 means edge weights are included.
    // Each adjacency entry is written as: neighbor weight.
    string weightedFile = originalDualFile ~ ".boundary_weighted";

    // Count undirected cell-cell interfaces for the METIS header.
    // The dual adjacency stores each interface twice, once from each cell.
    size_t nedges2 = 0;
    foreach(cell; grid.cells) nedges2 += cell.cell_cloud_ids.length;
    size_t nedges = nedges2 / 2;

    auto outFile = File(weightedFile, "w");
    outFile.writeln(grid.cells.length, " ", nedges, " 001");

    foreach(cell; grid.cells) {
        string line = "";
        int cellLayer = boundaryLayerIndex[cell.id];
        bool cellProtected = cellLayer <= layers;

        foreach(ncid; cell.cell_cloud_ids) {
            int neighbourLayer = boundaryLayerIndex[ncid];
            bool neighProtected = neighbourLayer <= layers;
            int w = 1;

            // Prefer partitions that keep wall-normal/boundary-layer stacks intact.
            // The graph distance from the protected wall tag acts as a layer index:
            //   |di-dj| == 1 : approximate wall-normal / layer-to-layer edge
            //   di == dj     : approximate tangential / same-layer edge
            // Give the layer-to-layer edges the large weight. This discourages cuts
            // through the boundary-layer thickness while still allowing tangential cuts.
            if (cellProtected && neighProtected) {
                int layerJump = abs(cellLayer - neighbourLayer);
                if (layerJump == 1) {
                    w = normalWeight;
                } else if (layerJump == 0) {
                    w = tangentialWeight;
                } else {
                    // Rare for a normal dual graph, but keep mild protection if it occurs.
                    w = tangentialWeight;
                }
            } else if (cellProtected || neighProtected) {
                // Do not strongly penalize the outer edge of the protected band; otherwise
                // METIS may simply move the cut to the band boundary.
                w = 1;
            }

            if (w < 1) w = 1;
            line ~= format("%d %d ", ncid + 1, w);
        }
        outFile.writeln(line.strip());
    }
    outFile.close();
    return weightedFile;
}

string applyBoundaryProtectedWeighting(string dualFormatFile,
                                          ref Domain grid,
                                          BoundaryProtectionOptions protection,
                                          ref bool cellAdjacencyLoaded) {
    if (protection.tags.length == 0 || protection.layers <= 0) {
        return dualFormatFile;
    }

    writeln("-- Applying boundary-protected METIS edge weights");
    writeln("-- -- protected tags: ", protection.tags.join(","));
    writeln("-- -- protected layers: ", protection.layers);
    writeln("-- -- normal weight: ", protection.normalWeight,
            ", tangential weight: ", protection.tangentialWeight);

    loadCellAdjacencyFromDualGraph(dualFormatFile, grid);
    cellAdjacencyLoaded = true;

    bool[] isBoundaryAdjacentCell = findProtectedBoundaryAdjacentCells(grid, protection.tags);

    size_t nBoundaryAdjacentCells = 0;
    foreach(isBoundaryAdjacent; isBoundaryAdjacentCell) {
        if (isBoundaryAdjacent) {
            nBoundaryAdjacentCells += 1;
        }
    }
    if (nBoundaryAdjacentCells == 0) {
        writeln("-- -- WARNING: no cells found adjacent to requested protected boundary tags; using unweighted dual graph.");
        return dualFormatFile;
    }
    writeln("-- -- protected boundary-adjacent cells: ", nBoundaryAdjacentCells);

    int[] boundaryLayerIndex = computeBoundaryLayerIndex(grid, isBoundaryAdjacentCell, protection.layers);
    size_t nCellsInProtectedLayerBand = 0;
    foreach(layerIndex; boundaryLayerIndex) {
        if (layerIndex <= protection.layers) {
            nCellsInProtectedLayerBand += 1;
        }
    }
    writeln("-- -- cells inside protected layer band: ", nCellsInProtectedLayerBand);

    return writeBoundaryWeightedDualGraph(dualFormatFile,
                                          grid,
                                          boundaryLayerIndex,
                                          protection.layers,
                                          protection.normalWeight,
                                          protection.tangentialWeight);
}
...
void assertPartitionsAreConnected(ref Domain grid, size_t nBlocks) {
    size_t[][] cellsInBlock;
    cellsInBlock.length = nBlocks;

    foreach (cell; grid.cells) {
        cellsInBlock[cell.block_id] ~= cell.id;
    }

    bool[] visited;
    visited.length = grid.cells.length;

    foreach (blockId; 0 .. nBlocks) {
        bool foundComponent = false;

        foreach (cellId; cellsInBlock[blockId]) {
            if (visited[cellId]) {
                continue;
            }
            ...
            if (foundComponent) {
                throw new Exception(
                    format(
                        "\n" ~
                        "Disconnected partition detected\n" ~
                        "-------------------------------\n" ~
                        "Block: %d\n\n" ~
                        "Reason:\n" ~
                        "  This block contains a disconnected island.\n" ~
                        "  A disconnected island is a group of cells assigned to the same block\n" ~
                        "  but not connected to the rest of that block through cell neighbours.\n\n" ~
                        "Suggested fixes:\n" ~
                        "  - Reduce the number of blocks\n" ~
                        "  - Try changing --seed\n" ~
                        "  - Increase --ncuts and/or --niter\n" ~
                        "  - Use --ptype=kway with --contig=true\n" ~
                        "  - Reduce boundary-protection weights\n",
                        blockId
                    )
                );
            }
            ...
            size_t[] cellsToVisit = [cellId];
            visited[cellId] = true;

            for (size_t nextCellToVisit = 0; nextCellToVisit < cellsToVisit.length; nextCellToVisit++) {
                size_t cid = cellsToVisit[nextCellToVisit];

                foreach (neighbourId; grid.cells[cid].cell_cloud_ids) {
                    if (grid.cells[neighbourId].block_id == blockId && !visited[neighbourId]) {
                        visited[neighbourId] = true;
                        cellsToVisit ~= neighbourId;
                    }
                }
            }
        }
...
void constructGridBlocks(bool reorder, string meshFile, string mappedCellsFilename, string partitionFile, string dualFile, ref Domain grid, ref Block[] gridBlocks, bool cellAdjacencyLoaded) {
    ...
    // Boundary-protected weighting may have already loaded this before partitioning.
    if (!cellAdjacencyLoaded) {
        writeln("-- -- Reading cell interconnectivity from dual format file");
        loadCellAdjacencyFromDualGraph(dualFile, grid);
    }

    assertPartitionsAreConnected(grid, grid.nblocks);
...
int main(string[] args){
    ...
    bool cellAdjacencyLoaded = false;
    string metisPartitionInputFile = dualFormatFile;
    if (nparts > 1) {
        metisPartitionInputFile = applyBoundaryProtectedWeighting(dualFormatFile,
                                                                  grid,
                                                                  cliOptions.protection,
                                                                  cellAdjacencyLoaded);
    }
    ...
    if (nparts > 1) {
        partitionFile = partitionDual(metisPartitionInputFile, nparts, metisOptions);
    } else {
    ...
    Block[] gridBlocks;
    constructGridBlocks(reorder, inputMeshFile, mappedCellsFilename, partitionFile, dualFormatFile, grid, gridBlocks, cellAdjacencyLoaded);
    ...
    cleanDir(dualFormatFile, partitionFile, metisFormatFile, metisPartitionInputFile);
