...
from pitot3.cli import run_pitot3
from pitot3.utils.classes import (
    pitot3_remote_run_creator,
    pitot3_pickle_output_file_creator,
    pitot3_json_output_file_loader,
    Facility,
    cleanup_function,
)
...
def run_pitot3_condition_builder(
    config_dict={},
    ...
    print("-" * 60)
    print(
        f"Running PITOT3 Condition Builder Version: {CONDITION_BUILDER_VERSION_STRING}."
    )
    print("-" * 60)
    ...
    if "condition_builder_cleanup" in condition_builder_config_dict:
        condition_builder_cleanup = condition_builder_config_dict[
            "condition_builder_cleanup"
        ]
    else:
        condition_builder_cleanup = False
    ...
    if condition_builder_cleanup:
        print("-" * 60)
        print(
            "Removing any evidence of an old condition builder run with the same name from the folder."
        )
        print(
            f"(This will remove any file or folder related to the basename of {base_output_filename}"
        )
        print("-" * 60)

        # this is teh end of the filename, the base filename will be added to it
        filenames_to_remove_list = [
            "_condition_builder_output.csv",
            "_final_result_dict_output.json",
            "_final_result_dict_output.pickle",
            "_individual_log_and_result_files.zip",
        ...
        remove_condition_builder_folders(
            mode="remove all folders",
    ...
    if (
        "driver_condition" in condition_builder_config_dict
        and "driver_condition_list" in condition_builder_config_dict
    ):
        raise Exception(
            "pitot3_condition_builder() driver_condition and driver_condition_List cannot be in the same config file."
        )
        ...
        print(
            f"Only the driver condition {condition_builder_config_dict['driver_condition']} will be tested."
        )
        ...
        if len(driver_condition_list) == 1:
            print(
                f"Only the driver condition {driver_condition_list[0]} will be tested."
            )
        else:
            print(
                f"{len(driver_condition_list)} driver conditions will be tested: {driver_condition_list}"
            )
    ...
    for fill_state in fill_states:
        if (
            f"p{fill_state}" in condition_builder_config_dict
            and f"p{fill_state}_range" in condition_builder_config_dict
        ):
            raise Exception(
                f"pitot3_condition_builder() p{fill_state} and p{fill_state}_range cannot be in the same config file."
            )
            ...
            f"p{fill_state}_range" in condition_builder_config_dict
            and f"p{fill_state}_no_of_samples" in condition_builder_config_dict
            ...
            fill_pressure_spacing = condition_builder_config_dict[
                f"p{fill_state}_spacing"
            ]
            fill_pressure_no_of_samples = condition_builder_config_dict[
                f"p{fill_state}_no_of_samples"
            ]
            ...
            if fill_pressure_spacing == "linear":
                ...
                locals()[f"p{fill_state}_values"] += list(
                    np.linspace(
                        fill_pressure_range[0],
                        fill_pressure_range[1],
                        fill_pressure_no_of_samples,
                    )
                )
            ...
            print(
                f"p{fill_state} will be varied from {fill_pressure_range[0]} to {fill_pressure_range[1]} Pa {fill_pressure_spacing_word} using {fill_pressure_no_of_samples} samples."
            )

        else:
            raise Exception(
                f"pitot3_condition_builder() Enough values have not been provided to make a p{fill_state} list. Need p{fill_state} or p{fill_state}_range, p{fill_state}_no_of_samples, AND p{fill_state}_spacing"
            )
    ...
    print("-" * 60)
    print(
        "Checking for any excess old condition builder folders in the simulation folder and cleaning them up."
    )
    print("-" * 60)
    ...
    remove_condition_builder_folders(
        mode="conserve folders",
        base_output_filename=base_output_filename,
        folder_list=test_names,
        cwd=".",
    )
    ...
    if no_of_processes == 1:
        for test_name in test_names:
            pitot3_condition_builder_test_run(
                test_name, changing_input_config_dict, variables_we_iterate_through
            )
    else:
        ...
        input_values = zip(
            test_names,
            itertools.repeat(changing_input_config_dict),
            itertools.repeat(variables_we_iterate_through),
        )
    ...
    print("-" * 60)
    print(
        f"This total calculation took {total_time:.2f} s ({total_time / 60.0:.2f} mins, {total_time / 3600.0:.2f} hours)."
    )
    print(
        "(Noting that this does not include the time taken for any runs performed before the last restart.)"
    )
    print("-" * 60)
    ...
    condition_builder_output_filename = (
        f"{base_output_filename}_condition_builder_output.csv"
    )
    ...
    with open(condition_builder_output_filename, "w") as condition_builder_output_file:
        ...
        for test_name in test_names:
            ...
            if json_filename in files_in_the_current_run_directory:
                ...
                print(
                    f"{test_name} does not have a .json output file so it must have failed."
                )
    ...
    json_results_dict_output_filename = (
        f"{base_output_filename}_final_result_dict_output.json"
    )
    ...
    pickle_results_dict_output_filename = (
        f"{base_output_filename}_final_result_dict_output.pickle"
    )

    # may as well use the pitot3 pickle output function for this...
    pitot3_pickle_output_file_creator(
        results_objects_dict, pickle_results_dict_output_filename
    )
    ...
    try:
        ...
        with zipfile.ZipFile(
            zipfile_name, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9
        ) as zf:
            for dirname, subdirs, files in os.walk(cwd):
                ...
                if len(dirname) > len(cwd):  # we're not the working directory...
                    dirname_for_comparison = dirname[
                        len(cwd) + 1 :
        ...
        print(e)
        print(
            "We failed to zip up the files, will try again without specifying the compression level"
        )
    ...
    remove_condition_builder_folders(
        mode="remove folders", folder_list=test_names, cwd="."
    )
    ...
    print("-" * 60)
    print(
        "Printing summary of the total calculation to the screen and to a text document."
    )
    print("-" * 60)

    condition_builder_summary_filename = (
        f"{base_output_filename}_condition_builder_summary.txt"
    )

    with open(
        condition_builder_summary_filename, "w"
    ) as condition_builder_summary_file:
        ...
        for output_stream in output_list:
            ...
            no_of_successful_simulations = no_of_simulations_to_be_ran - len(
                unsuccessful_simulations
            )

            percentage_successful = (
                no_of_successful_simulations / no_of_simulations_to_be_ran
            ) * 100
            ...
            if len(driver_condition_list) > 1:
                ...
                summary_line_5 = (
                    f"Only the driver condition {driver_condition_list[0]} was tested."
                )

            print(summary_line_5, file=output_stream)

            variables_to_not_summarise_list = [
                "test_number",
                "driver_condition",
                "area_ratio",
                "secondary_driver_gas_gas_model",
                "secondary_driver_gas_name",
                "test_gas_gas_model",
                "test_gas_name",
                "accelerator_gas_gas_model",
                "accelerator_gas_name",
            ]
            ...
            for variable in results_dict.keys():
                ...
                if (
                    len(data_list) > 0
                ):  # ie. don't bother summarising if there is no numerical data there
...
def pitot3_condition_builder_test_run(
    test_name, changing_input_config_dict, variables_we_iterate_through
):
    """
    ...
    if (
        f"{test_name}.json" not in files_in_the_current_run_directory
        and f"{test_name}_failed.txt" not in files_in_the_current_run_directory
    ):
        # this is one of the output files, so the simulation has not been ran.
        run_simulation = True
    elif (
        f"{test_name}.json" not in files_in_the_current_run_directory
        and f"{test_name}_failed.txt" in files_in_the_current_run_directory
    ):
        ...
        if run_simulation:
            print(
                "There was a result in this folder but at least one of the variables was different between it and the new simulation."
            )
            ...
            with redirect_stdout(logfile):
                with redirect_stderr(logfile):
                    try:
                        ...
                        print(
                            f"The dummy file {failure_filename} will also be created in the folder so the program knows that the run failed."
                        )
...
def remove_condition_builder_folders(
    mode="remove all folders", base_output_filename=None, folder_list=None, cwd="."
):
    """
    ...
    for dirname, subdirs, files in os.walk(cwd):
        ...
        if len(dirname) > len(cwd):  # we're not the working directory...
            ...
            if mode == "remove all folders" and f"{base_output_filename}_" in dirname:
                ...
                mode == "conserve folders"
                and dirname_for_comparison not in folder_list
                and f"{base_output_filename}_" in dirname
            ):
                print(
                    f"Deleting the folder {dirname} as it is not in the list of simulation folders to conserve."
                )
                shutil.rmtree(dirname)
            elif mode == "remove folders" and dirname_for_comparison in folder_list:
                print(
                    f"Deleting the folder {dirname} as it is in the list of folders to delete."
                )
                shutil.rmtree(dirname)
