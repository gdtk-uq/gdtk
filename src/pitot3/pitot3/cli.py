...
from pitot3.utils.classes import (
    Facility,
    Driver,
    Diaphragm,
    Tube,
...
from pitot3.utils.classes import (
    pitot3_results_output,
    cleanup_function,
    pitot3_species_MW_dict_loader,
)

VERSION_STRING = "21-Feb-2025"
...
def run_pitot3(
    ...
    pitot3_data_folder="$PITOT3_DATA",
    default_config_yaml_filename="PITOT3_default_config.yaml",
    just_setup_simulation=False,
    verbose=True,
):
    """
    ...
    default_config_file = open(
        os.path.expandvars(pitot3_data_folder + "/" + default_config_yaml_filename)
    )
    ...
    if nozzle_flag:
        if "area_ratio" in config_data:
            ...
            if verbose:
                print(
                    f"User has not specified an area ratio, so the geometric area ratio of this facility's nozzle ({area_ratio}) will be used."
                )
    ...
    if driver_condition_name != "custom_from_dict":
        if driver_condition_name != "custom":
            ...
            selected_facility_driver_conditions_folder = (
                facilities_folder + "/" + facility.get_driver_conditions_folder()
            )
            driver_condition_file_location = (
                selected_facility_driver_conditions_folder
                + "/"
                + driver_condition_name
                + ".yaml"
            )
        else:
            if verbose:
                print(
                    f"Using custom driver condition from the file {config_data['driver_condition_filename']}."
                )
        ...
        if os.path.isfile(os.path.expandvars(driver_condition_file_location)):
            ...
            driver_condition_input_data = yaml.load(
                driver_yaml_file, Loader=yaml.FullLoader
            )
        elif (
            not os.path.isfile(os.path.expandvars(driver_condition_file_location))
            and driver_condition_name != "custom"
        ):
            if verbose:
                print(f"Your selected facility is {facility_name}. ")
                print(
                    f"Its driver conditions are stored in the folder {selected_facility_driver_conditions_folder}"
                )
                print(
                    f"Your selected driver condition ('{driver_condition_name}') does not appear to exist in that folder."
                )
                print(
                    "Below is the list of driver conditions in that folder. Are you sure you didn't mean one of those instead?"
                )

                selected_facility_driver_conditions_folder_files = os.listdir(
                    os.path.expandvars(selected_facility_driver_conditions_folder)
                )
            ...
            raise Exception(
                f"pitot3.py: Specified facility driver condition does not exist in the {facility_name} facility driver condition folder."
            )
            ...
            if verbose:
                print(
                    f"Your selected driver condition filename ('{driver_condition_file_location}') does not appear to exist."
                )
                print(
                    "Below is the list of files in your current working directory. Are you sure you didn't mean one of these files?"
                )
            ...
            raise Exception(
                "pitot3.py: Specified custom facility driver condition file does not exist in the location the user has specified."
            )
        else:
            raise Exception(
                "pitot3.py: There is some issue loading the driver condition."
            )

    else:
        if verbose:
            print(
                "Using driver condition specified in a dictionary in the input config."
            )
    ...
    driver = Driver(
        driver_condition_input_data,
        p_0=p_0,
        T_0=T_0,
        preset_gas_models_folder=preset_gas_models_folder,
        outputUnits=outputUnits,
        species_MW_dict=species_MW_dict,
    ...
    if verbose:
        ...
        if state4.get_gas_state().gmodel.type_str == "CEAGas":
            print(
                f"Driver gas composition is {state4.get_reduced_composition_single_line_output_string()} ({state4.get_gamma_and_R_string()})."
            )
        else:
    ...
    if verbose:
        if v4 == 0.0:
            print(
                f"Driver rupture conditions are p4 = {state4.get_gas_state().p / 1.0e6:.2f} MPa, T4 = {state4.get_gas_state().T:.2f} K, "
    ...
    if secondary_driver_flag:
        ...
        secondary_driver_fill_state_name = config_data[
            "secondary_driver_fill_state_name"
        ]
        ...
        if verbose:
            if secondary_driver_gas_gas_model != "custom":
                print(
                    f"Secondary driver gas is {secondary_driver_gas_name}. Secondary driver gas gas model is {secondary_driver_gas_gas_model}."
                )
            else:
                print(
                    f"Using custom secondary driver gas from the file {secondary_driver_gas_filename}."
                )
                print(
                    f"Secondary driver gas gas model is {secondary_driver_gas_gas_model}."
                )

            print(
                f"Secondary driver fill pressure (p{secondary_driver_fill_state_name[1:]}) is {psd1:.2f} Pa."
            )
            print(
                f"Secondary driver fill temperature (T{secondary_driver_fill_state_name[1:]}) is {Tsd1:.2f} K."
            )
    ...
    if verbose:
        if test_gas_gas_model != "custom":
            print(
                f"Test gas is {test_gas_name}. Test gas gas model is {test_gas_gas_model}."
            )
        else:
        ...
        print(
            f"Shock tube fill pressure (p{shock_tube_fill_state_name[1:]}) is {p1:.2f} Pa."
        )
        print(
            f"Shock tube fill temperature (T{shock_tube_fill_state_name[1:]}) is {T1:.2f} K."
        )

    if mode in [
        "fully_experimental",
        "experimental_shock_tube_theoretical_acceleration_tube",
    ]:
    ...
    if facility_type == "expansion_tube":
        ...
        acceleration_tube_fill_state_name = config_data[
            "acceleration_tube_fill_state_name"
        ]
        ...
        if verbose:
            if accelerator_gas_gas_model != "custom":
                print(
                    f"Accelerator gas is {accelerator_gas_name}. Accelerator gas gas model is {accelerator_gas_gas_model}."
                )
            else:
                print(
                    f"Using custom accelerator gas from the file {accelerator_gas_filename}."
                )
                print(f"Accelerator gas gas model is {accelerator_gas_gas_model}.")

            print(
                f"Acceleration tube fill pressure (p{acceleration_tube_fill_state_name[1:]}) is {p5:.2f} Pa."
            )
            print(
                f"Acceleration tube fill temperature (T{acceleration_tube_fill_state_name[1:]}) is {T5:.2f} K."
            )

        if mode in [
            "fully_experimental",
            "theoretical_shock_tube_experimental_acceleration_tube",
        ]:
    ...
    primary_diaphragm_cfg = {
        "diaphragm_name": config_data["primary_diaphragm_name"],
    ...
    primary_diaphragm_cfg["diaphragm_entrance_state_name"] = gas_path[
        -1
    ].get_exit_state_name()
    ...
    if secondary_driver_flag:
        if facility:
            secondary_driver_length, secondary_driver_diameter = (
                facility.get_secondary_driver_length_and_diameter()
            )
        else:
        ...
        secondary_driver_expansion_factor = config_data[
            "secondary_driver_expansion_factor"
        ]

        secondary_driver_unsteady_expansion_steps = config_data[
            "secondary_driver_unsteady_expansion_steps"
        ]
        ...
        secondary_driver_shocked_state_name = config_data[
            "secondary_driver_shocked_state_name"
        ]
        secondary_driver_unsteadily_expanded_state_name = config_data[
            "secondary_driver_unsteadily_expanded_state_name"
        ]

        secondary_driver = Tube(
            tube_name=secondary_driver_tube_name,
            tube_length=secondary_driver_length,
            tube_diameter=secondary_driver_diameter,
            fill_pressure=psd1,
            fill_temperature=Tsd1,
            fill_gas_model=secondary_driver_gas_gas_model,
            fill_gas_name=secondary_driver_gas_name,
            fill_gas_filename=secondary_driver_gas_filename,
            ...
            expand_to=secondary_driver_expand_to,
            expansion_factor=secondary_driver_expansion_factor,
            preset_gas_models_folder=preset_gas_models_folder,
            unsteady_expansion_steps=secondary_driver_unsteady_expansion_steps,
            vs_guess_1=vsd_guess_1,
            vs_guess_2=vsd_guess_2,
            vs_limits=vsd_limits,
            vs_tolerance=vsd_tolerance,
            vs_max_iterations=vsd_max_iterations,
            outputUnits=outputUnits,
            species_MW_dict=species_MW_dict,
        )
        ...
        secondary_diaphragm_cfg = {
            "diaphragm_name": config_data["secondary_diaphragm_name"],
            "diaphragm_type": config_data["secondary_diaphragm_diaphragm_type"],
            "Mr_input": config_data["secondary_diaphragm_Mr"],
            "velocity_loss_factor": config_data[
                "secondary_diaphragm_velocity_loss_factor"
            ],
        }

        # now we add the diaphragm entrance state by getting the shock tube exit state...
        secondary_diaphragm_cfg["diaphragm_entrance_state_name"] = gas_path[
            -1
        ].get_exit_state_name()
        secondary_diaphragm_cfg["diaphragm_entrance_state"] = gas_path[
            -1
        ].get_exit_state()
    ...
    if facility:
        shock_tube_length, shock_tube_diameter = (
            facility.get_shock_tube_length_and_diameter()
        )
    else:
    ...
    shock_tube_unsteady_expansion_steps = config_data[
        "shock_tube_unsteady_expansion_steps"
    ]
    ...
    shock_tube_unsteadily_expanded_state_name = config_data[
        "shock_tube_unsteadily_expanded_state_name"
    ]

    shock_tube = Tube(
        tube_name=shock_tube_tube_name,
        tube_length=shock_tube_length,
        tube_diameter=shock_tube_diameter,
        fill_pressure=p1,
        fill_temperature=T1,
        fill_gas_model=test_gas_gas_model,
        fill_gas_name=test_gas_name,
        fill_gas_filename=test_gas_filename,
        fill_state_name=shock_tube_fill_state_name,
        shocked_fill_state_name=shock_tube_shocked_state_name,
        entrance_state_name=gas_path[-1].get_exit_state_name(),
        entrance_state=gas_path[-1].get_exit_state(),
        unsteadily_expanded_entrance_state_name=shock_tube_unsteadily_expanded_state_name,
        expand_to=shock_tube_expand_to,
        expansion_factor=shock_tube_expansion_factor,
        preset_gas_models_folder=preset_gas_models_folder,
        unsteady_expansion_steps=shock_tube_unsteady_expansion_steps,
        vs_guess_1=vs1_guess_1,
        vs_guess_2=vs1_guess_2,
        vs_limits=vs1_limits,
        vs_tolerance=vs1_tolerance,
        vs_max_iterations=vs1_max_iterations,
        outputUnits=outputUnits,
        species_MW_dict=species_MW_dict,
    )
    ...
    if not secondary_driver_flag:
        ...
        tertiary_diaphragm_cfg = {
            "diaphragm_name": config_data["tertiary_diaphragm_name"],
            "diaphragm_type": config_data["tertiary_diaphragm_diaphragm_type"],
            "Mr_input": config_data["tertiary_diaphragm_Mr"],
            "velocity_loss_factor": config_data[
                "tertiary_diaphragm_velocity_loss_factor"
            ],
        }

        # now we add the diaphragm entrance state by getting the shock tube exit state...
        tertiary_diaphragm_cfg["diaphragm_entrance_state_name"] = gas_path[
            -1
        ].get_exit_state_name()
        tertiary_diaphragm_cfg["diaphragm_entrance_state"] = gas_path[
            -1
        ].get_exit_state()
    ...
    if facility_type == "expansion_tube":
        ...
        if facility:
            acceleration_tube_length, acceleration_tube_diameter = (
                facility.get_acceleration_tube_length_and_diameter()
            )
        else:
        ...
        acceleration_tube_expansion_factor = config_data[
            "acceleration_tube_expansion_factor"
        ]

        acceleration_tube_unsteady_expansion_steps = config_data[
            "acceleration_tube_unsteady_expansion_steps"
        ]
        ...
        acceleration_tube_shocked_state_name = config_data[
            "acceleration_tube_shocked_state_name"
        ]
        acceleration_tube_unsteadily_expanded_state_name = config_data[
            "acceleration_tube_unsteadily_expanded_state_name"
        ]
        ...
        acceleration_tube = Tube(
            tube_name=acceleration_tube_tube_name,
            tube_length=acceleration_tube_length,
            tube_diameter=acceleration_tube_diameter,
            fill_pressure=p5,
            fill_temperature=T5,
            fill_gas_model=accelerator_gas_gas_model,
            fill_gas_name=accelerator_gas_name,
            fill_gas_filename=accelerator_gas_filename,
            ...
            expand_to=acceleration_tube_expand_to,
            expansion_factor=acceleration_tube_expansion_factor,
            preset_gas_models_folder=preset_gas_models_folder,
            unsteady_expansion_steps=acceleration_tube_unsteady_expansion_steps,
            vs_guess_1=vs2_guess_1,
            vs_guess_2=vs2_guess_2,
            vs_limits=vs2_limits,
            vs_tolerance=vs2_tolerance,
            vs_max_iterations=vs2_max_iterations,
            outputUnits=outputUnits,
            species_MW_dict=species_MW_dict,
        )
    ...
    if nozzle_flag:
        ...
        if facility_type == "expansion_tube":
            expansion_tube_nozzle_expansion_minimum_p2_over_p1 = config_data[
                "expansion_tube_nozzle_expansion_minimum_p2_over_p1"
            ]
        else:
            expansion_tube_nozzle_expansion_minimum_p2_over_p1 = None

        maximum_temp_for_room_temperature_only_gmodel = config_data[
            "maximum_temp_for_room_temperature_only_gmodel"
        ]
        ...
        nozzle = Nozzle(
            entrance_state_name=gas_path[-1].get_exit_state_name(),
            entrance_state=gas_path[-1].get_exit_state(),
            exit_state_name=nozzle_exit_state_name,
    ...
    test_section_post_shock_state_name = config_data[
        "test_section_post_shock_state_name"
    ]
    ...
    if "wedge_angle_degrees" in config_data:
        ...
        test_section.calculate_post_wedge_shock_state(
            wedge_angle_degrees=wedge_angle_degrees
        )
    ...
    states_dict = pitot3_results_output(
        config_data,
        gas_path,
        object_dict,
        generate_output_files=config_data["generate_output_files"],
    )

    if config_data["cleanup_run_files"]:
        # cleanup temporary files (and potentially generated gas models) before exiting...
        cleanup_function(
            cleanup_generated_gas_models=config_data["cleanup_generated_gas_models"]
        )
