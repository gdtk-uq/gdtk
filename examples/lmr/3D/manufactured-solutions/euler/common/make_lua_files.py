...
fxmom = diff(rho*u, t) + diff(rho*u*u+p, x) + diff(rho*u*v, y) + diff(rho*u*w, z)
fymom = diff(rho*v, t) + diff(rho*v*u, x) + diff(rho*v*v+p, y) + diff(rho*v*w, z)
fzmom = diff(rho*w, t) + diff(rho*w*u, x) + diff(rho*w*v, y) + diff(rho*w*w+p, z)
fe = diff(rho*et, t) + diff(rho*u*et+p*u, x) + diff(rho*v*et+p*v, y) + diff(rho*w*et+p*w, z)
