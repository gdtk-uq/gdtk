...
# Vorticity-source variant from kwTurbulenceModel.source_terms() for 2D Cartesian flow
P_K = mu_t*(diff(v, x) - diff(u, y))*(diff(v, x) - diff(u, y)) - \
   2.0/3.0*rho*tke*(diff(u, x) + diff(v, y))
P_K = Min(P_K, 25.0*D_K)
WWS = 0.0
cross_diff = diff(tke, x)*diff(omega, x) + diff(tke, y)*diff(omega, y)
sigma_d = Min(0.0, cross_diff)
P_W = alpha*omega/Max(1.0e-08, tke)*P_K + sigma_d*rho/Max(1.0, omega)*cross_diff
