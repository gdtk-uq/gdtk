...
fxmom = diff(rho*u, t) + diff(rho*u*u+p, x) + diff(rho*u*v, y)
fymom = diff(rho*v, t) + diff(rho*v*u, x) + diff(rho*v*v+p, y)
fe = diff(rho*et, t) + diff(rho*u*et+p*u, x) + diff(rho*v*et+p*v, y)
