...
#
#         04-Oct-2025
#         Updated script to work with Eilmer 5 using sympy 1.9,
#         and modified for the k-log(omega) model
#         author: KAD
#
...
omega_t = Max(exp(omega), C_lim*(2.0*S_bar_squared/beta_star)**0.5)
...
sigma = 0.5
sigma_star = 0.6
D_K = beta_star*rho*tke*exp(omega)
...
grad2 = diff(omega, x)*diff(omega, x) + diff(omega, y)*diff(omega, y)
P_W = alpha/Max(1.0e-08, tke)*P_K + sigma_d*rho/Max(1.0, exp(omega))*cross_diff + (mu + sigma*mu_t)*grad2
X_w = abs(WWS/(beta_star*exp(omega))**3.0)
f_beta = (1.0 + 85.0*X_w)/(1.0 + 100.0*X_w)
beta = beta_0*f_beta
D_W = beta*rho*exp(omega)
...
fe = diff(rho*(et+tke), t) + diff(rho*u*(ht+tke), x) + diff(rho*v*(ht+tke), y) + diff(qlx+qtx, x) + diff(qly+qty, y) - \
     ...
     diff((mu+sigma_star*rho*tke/exp(omega))*diff(tke, x), x) - diff((mu+sigma_star*rho*tke/exp(omega))*diff(tke, y), y)
ftke = diff(rho*tke, t) + diff(rho*u*tke, x) + diff(rho*v*tke, y) - P_K + D_K - diff((mu+sigma_star*rho*tke/exp(omega))*diff(tke, x), x) - \
       diff((mu+sigma_star*rho*tke/exp(omega))*diff(tke, y), y)
fomega = diff(rho*omega, t) + diff(rho*u*omega, x) + diff(rho*v*omega, y) - P_W + D_W - diff((mu+sigma*rho*tke/exp(omega))*diff(omega, x), x) - \
         diff((mu+sigma*rho*tke/exp(omega))*diff(omega, y), y)
    ...
    taskList = [ {'fName': "udf-source-terms.lua", 'tName': "udf-source-template.lua",
                 ...
                 {'fName': "ref-soln.lua", 'tName': "ref-soln-template.lua",
                  'varList': [("tab.rho", rho), ("tab.p", p), ("tab.T", T),
                              ...
                              ("tab.mu_t", mu_t), ("tab.k_t", k_t), ("tab['tq-tke']", tke), ("tab['tq-omega']", omega)]},
