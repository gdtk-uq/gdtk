-- nz.lua : T4 Mach 7 nozzle steady-state, lmr example with k omega
-- References:
...
config.turbulence_model = "k_omega_vorticity"
...
tke_inf = 1.5 * (V6 * 0.05)^2
mu_t_inf = 1.0 * state6.mu -- Wilson has 100.0 here which crashes the modern solver. What gives?
omega_inf = state6.rho * tke_inf / mu_t_inf

inflow = FlowState:new{p=gas_inf.p, T=gas_inf.T, velx=V6, massf=gas_inf.massf, tke=tke_inf, omega=omega_inf}
outflow = FlowState:new{p=state7.p, T=state7.T, velx=0.0, massf=gas_inf.massf, tke=tke_inf, omega=omega_inf}
vacuum = FlowState:new{p=state7.p, T=state7.T, velx=0.0, massf=gas_inf.massf, tke=tke_inf, omega=omega_inf}
print("inflow: ", inflow)
...
expand_patch = ControlPointPatch:new{north=bd, east=cd, south=ac, west=ab,
...
turbZone_botLeftCorner = Vector3:new{x=0.05, y=y0}
turbZone_topRightCorner = Vector3:new{x=2.0, y=0.5}
...
NewtonKrylovPhase:new{
   ...
   use_auto_cfl = true,
   threshold_relative_residual_for_cfl_growth = 0.05,
   start_cfl = 0.5,
   max_cfl = 1.0e4,
