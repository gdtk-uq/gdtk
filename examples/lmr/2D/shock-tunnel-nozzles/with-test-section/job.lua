-- nz.lua : T4 Mach 7 nozzle steady-state, lmr example with test section
-- References:
...
vacuum = FlowState:new{p=state7.p, T=state7.T, velx=0.0, massf=gas_inf.massf, nuhat=nuhat_inf}
print("inflow: ", inflow)
...
test_section_length = 500e-3
test_section_radius = 0.225
ts_niv = 86
ts_njv = 32
x_exit = d.x
y_exit = d.y
...
ts_core_region = CoonsPatch:new{
    ...
    p10=Vector3:new{x=x_exit+test_section_length, y=0.0},
    p11=Vector3:new{x=x_exit+test_section_length, y=y_exit},
    p01=Vector3:new{x=x_exit, y=y_exit}}
ts_wall_region = CoonsPatch:new{
    p00=Vector3:new{x=x_exit, y=y_exit},
    p10=Vector3:new{x=x_exit+test_section_length, y=y_exit},
    p11=Vector3:new{x=x_exit+test_section_length, y=test_section_radius},
    p01=Vector3:new{x=x_exit, y=test_section_radius}}

print("A")
ts_core_grid = StructuredGrid:new{psurface=ts_core_region, niv=ts_niv, njv=nnj,
    cfList = {
              west=GeometricFunction:new{a=0.00005, r=1.3, N=nnj, reverse=true},
              east=GeometricFunction:new{a=0.00005, r=1.3, N=nnj, reverse=true}
          }
}
ts_wall_grid = StructuredGrid:new{psurface=ts_wall_region, niv=ts_niv, njv=ts_njv,
    cfList = {
              west=GeometricFunction:new{a=0.005, r=1.3, N=ts_njv, reverse=false},
              east=GeometricFunction:new{a=0.005, r=1.3, N=ts_njv, reverse=false}
          }
}
