# prepare-transient-profile.py
#
# Run with commands like:
# $ prep-gas ideal-air.inp ideal-air.gas
# $ python3 prepare-transient-profile.py
#
# PJ, 2024-01-18 adapted from classic-shock-tube.py
...
#
# Write the transient-profile data file as a zip archive.
import json
import zipfile
import zlib
# Time instances, in seconds.
ts = [0.0, 100.0e-6, 101.0e-6]
# Positions of the sample points in all profiles.
x = 0.5
ys = [0.025, 0.075]
# Note that, for this single-species simulation,
# we omit the species-massfraction data
# because it will be 1.0 and ignored anyway.
varnames = ['pos.x', 'pos.y', 'p', 'T', 'vel.x', 'vel.y']
metadata = {
    'npoints': 2,
    'nspecies': 1,
    'nmodes': 0,
...
#
zf = zipfile.ZipFile('transient-profile.zip', mode='w',
                     compression=zipfile.ZIP_DEFLATED)
zf.writestr('metadata.json', json.dumps(metadata))
#
# When writing the flow profile for each time instance,
# we include the column labels as a header line
# because it will be convenient to have those names
# when manually poking around in the archive.
# The transient profile reader will ignore them.
head_str = ''
for name in varnames: head_str += ' ' + name
#
content0 = head_str + '\n'
for y in ys:
    content0 += '%g %g %g %g %g %g\n' % (x, y, states[1].p, states[1].T, 0.0, 0.0)
zf.writestr('data.0', content0)
#
content1 = head_str + '\n'
for y in ys:
    content1 += '%g %g %g %g %g %g\n' % (x, y, states[1].p, states[1].T, 0.0, 0.0)
zf.writestr('data.1', content1)
#
content2 = head_str + '\n'
for y in ys:
    content2 += '%g %g %g %g %g %g\n' % (x, y, states[2].p, states[2].T, v2g, 0.0)
zf.writestr('data.2', content2)
#
zf.close()
