...
-- UPDATE:
--    RJG [2025-12-22]
--    This version uses tapered blending to
--    go from AUSMDV to ASF. The resulting flow field
--    is not particularly good because this grid
--    is far too coarse for ASF. However, as a demo
--    of using the tapered blending, it suffices.
...
NewtonKrylovGlobalConfig{
   ...
   max_newton_steps = 150,
   stop_on_relative_residual = 1.0e-12,
   number_of_phases = 3,
   max_steps_in_initial_phases = {30, 70},
   ...
   preconditioner = "ilu",
   ilu_fill = 0,
   preconditioner_flux_calculator = "ausmdv",
   use_tapered_blending = true,
   blending_flux_calculator = "ausmdv",
   total_snapshots = 3,
...
NewtonKrylovPhase:new{
   -- In first phase, use belnding calculator ONLY
   use_blending_flux_calculator = true,
...
NewtonKrylovPhase:new{
   -- In second phase, turn on blending
   tapered_blending = true,
   start_cfl = 5.0,
}

NewtonKrylovPhase:new{
   -- In third phase, use high order ASF only
