...
--    of the gas.
-- We can not pick up the '0' snapshot for the initial energy so
-- we are using the first snapshot, this requires the piston velocity
...
local nb = 2
local fsolStart = FlowSolution:new{dir=".", snapshot=1, nBlocks=nb}
local fsolEnd = FlowSolution:new{dir=".", snapshot=8, nBlocks=nb}
...
local gmodel = GasModel:new{"ideal-air.gas"}
...
for blk = 0, nb-1 do
   for i = 0, fsolStart:get_nic(0)-1 do
      for j = 0, fsolStart:get_njc(0)-1 do
         ...
         cell0mass = cell0.vol * 2 * math.pi * cell0.rho
         ...
         -- Calculate the mass
         massStart = massStart + cell0.vol * 2 * math.pi * cell0.rho
         -- Now the last time step
         cell1 = fsolEnd:get_cell_data{ib=blk,i=i,j=j}
         cell1mass = cell1.vol * 2 * math.pi * cell1.rho
         ...
         -- Calculate the mass
         massEnd = massEnd + cell1.vol * 2 * math.pi * cell1.rho
      end
   end
end
