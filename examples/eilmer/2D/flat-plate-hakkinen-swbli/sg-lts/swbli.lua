...
-- Update to use Kyles local time stepping, PJ 2019-10-26
--
...
factor = 4 -- We'll scale discretization off this value
...
if true then
   -- Try Kyle's local time stepping to get the calculation done more quickly.
   config.with_local_time_stepping = true
   config.residual_smoothing = true
   config.local_time_stepping_limit_factor = 5000
   config.residual_smoothing_type = 'implicit'
   config.residual_smoothing_weight = 0.5
   config.residual_smoothing_iterations = 6
   config.cfl_value = 0.80 -- lower than for transient solver
end
