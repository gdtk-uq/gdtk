-- t4m7_chem.lua
-- Tamara Sopek, 23-Nov-2020
config.title = "T4 Mach7 nozzle with air in chemical nonequilibrium."
print(config.title)
...
config.dt_plot = config.max_time * 10 -- make dt_plot larger than max_time for block-marching
--
config.reacting = true -- turn on reactions
config.reactions_file = '5sp-chem-model.lua'
...
-- Set up flow state.
nsp, nmodes, gm = setGasModel('5sp-gas-model.lua')
...
if massf_sum ~= 1.0 then
  ...
  print("Original mass sum from CEA = ", massf_sum)
  massf_throat['N2'] = massf_throat['N2'] + (1.0 - massf_sum)
end

-- The inflow for the simulation is at the nozzle throat.
inflow = FlowState:new{p=state6.p, T=state6.T, velx=V6, tke=throat_tke, 
