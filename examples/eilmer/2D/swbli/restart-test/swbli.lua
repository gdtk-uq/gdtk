-- swbli.lua
-- Test case for restarting an NK simulation
-- Notes:
--   Uses some weird settings, such as no preconditioner and a
--   frozen CFL value. It would be nice if we could dial out
--   the hysteresis in the solver and put these in.
-- @author: NNG (13/06/22)
...
cflist = {north=cfx, east=GeometricFunction:new{a=0.002, r=1.2, N=njv},
          south=cfx, west=GeometricFunction:new{a=0.004, r=1.2, N=njv}}
grid = StructuredGrid:new{psurface=patch, niv=niv, njv=njv, cfList=cflist}
blks = FBArray:new{grid=grid, nib=4, njb=2, fillCondition=inflow,
