-- compute-analytic-data.lua
-- Run with a command like:
-- $ e4shared --custom-script --script-file=compute-analytic-data.lua
-- Note that you need the gas-model file in the current directory and that
-- the file analytic.data will be written with content suitable for plotting
-- the profile at t=100 microseconds.
-- PJ, 2018-04-27, 2019-07-15
print("Compute the flow conditions expected in the high-performance shock tube.")
--
print("shock-tube fill conditions with ideal-helium driving eq-air")
gm = GasModel:new{'lut-plus-helium-gas-model.lua'}
states = {}
for i=1,4 do states[i] = GasState:new{gm} end
helium = {lut=0.0, ideal=1.0}
air = {lut=1.0, ideal=0.0}
states[1].p = 30.0e3; states[1].T = 300.0; states[1].massf=air -- driven tube, initial
states[2].p = 30.0e3; states[2].T = 300.0; states[2].massf=air -- intermediate, post-shock
states[3].p = 30.0e6; states[3].T = 3000.0; states[3].massf=helium -- intermediate, post-expansion
states[4].p = 30.0e6; states[4].T = 3000.0; states[4].massf=helium -- driver tube, initial
...
--
-- Make a record for plotting against the Eilmer4 simulation data.
