...
class TestSWP < Test::Unit::TestCase
  ...
  def test_1_run32
    ...
    cmd = 'sed -i s/asf/ldfss2/ swp.lua'
    o, e, s = Open3.capture3(*cmd.split)
    assert_equal(s.success?, true)
    ...
    cmd = "mkdir -p ldfss2/032"
    o, e, s = Open3.capture3(*cmd.split)
    assert_equal(s.success?, true)

    cmd = "mv config flow grid hist ideal-air.inp ideal-air.lua loads solid swp.lua ldfss2/032"
    o, e, s = Open3.capture3(*cmd.split)
    assert_equal(s.success?, true)
  end
  ...
  def test_2_run64
    ...
    cmd = "mkdir -p ldfss2/064"
    o, e, s = Open3.capture3(*cmd.split)
    assert_equal(s.success?, true)

    cmd = "mv config flow grid hist ideal-air.inp ideal-air.lua loads solid swp.lua ldfss2/064"
    o, e, s = Open3.capture3(*cmd.split)
    assert_equal(s.success?, true)
  end
  ...
  def test_3_run128
    ...
    cmd = "mkdir -p ldfss2/128"
    o, e, s = Open3.capture3(*cmd.split)
    assert_equal(s.success?, true)

    cmd = "mv config flow grid hist ideal-air.inp ideal-air.lua loads solid swp.lua ldfss2/128"
    o, e, s = Open3.capture3(*cmd.split)
    assert_equal(s.success?, true)
  end

  def test_4_order
    cmd = 'python3 scripts/post_process.py ldfss2/032 ldfss2/064 ldfss2/128'
    o, e, s = Open3.capture3(*cmd.split)
    assert_equal(s.success?, true)
    lines = o.split("\n")
    ...
    assert((rhoL0-1.95).abs < 1.0e-03, "Incorrect density L0 norm.")
    assert((rhoL2-2.03).abs < 1.0e-03, "Incorrect density L2 norm.")
    assert((velL0-2.02).abs < 1.0e-03, "Incorrect velocity L0 norm.")
    assert((velL2-2.07).abs < 1.0e-03, "Incorrect velocity L2 norm.")
  end
end
