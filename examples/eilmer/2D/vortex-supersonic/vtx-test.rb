...
class TestVtx < Test::Unit::TestCase
  ...
  def test_1_run
    ...
    assert((steps - 2740).abs < 3, "Failed to take correct number of steps.")
  end
  ...
  def test_2_post_L2_norms
    ...
    assert((pressure - 600.0).abs < 100.0, "Failed to see expected pressure error.")
    #
    ...
    assert((temperature - 0.8).abs < 0.10, "Failed to see expected temperature error.")
  end
end
