...
gm_air = GasModel('he-air-5sp-1T-gas-model.lua')
flow_air = GasFlow(gm_air)
flow_he = GasFlow(gm_he)
states = []
states.append(GasState(gm_he))  # [0] to interpolate inside expansion
states.append(GasState(gm_air))  # [1] driven air
states.append(GasState(gm_air))  # [2] driven air
states.append(GasState(gm_he))  # [3] expanded He
states.append(GasState(gm_he))  # [4] driver He
states[0].p = 0.80e6; states[0].T = 800.0 # spare, to be used later
states[1].p = 290.0;  states[1].T = 293.0; states[1].massf={'N2':0.78,'O2':0.22}  # driven tube, initial
states[2].p = 290.0;  states[2].T = 293.0; states[2].massf={'N2':0.78,'O2':0.22}  # intermediate, post-shock
states[3].p = 0.80e6; states[3].T = 800.0 # intermediate, post-expansion
states[4].p = 0.80e6; states[4].T = 800.0 # driver tube, initial
for gs in states:
    print("Before update: gs=", gs)
    gs.update_thermo_from_pT()
    print("After update: gs=", gs)
