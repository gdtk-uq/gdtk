#! /usr/bin/env ruby
# cone20-mpi-test.rb
# Tests for the conical-flow sharp-cone example, MPI execution.
...
class TestCone20_MPI < Test::Unit::TestCase
  def test_0_prep
  ...
  def test_1_run
    cmd = "mpirun -np 8 --oversubscribe e4mpi --run --job=cone20 --verbosity=1"
    o, e, s = Open3.capture3(*cmd.split)
    assert_equal(s.success?, true)
    steps = 0
    lines = o.split("\n")
  ...
  def test_3_post_pressure_on_cone_surface
    ...
    # puts "q_inf=#{q_inf} p=#{value_p} p_cone_surface=#{p_cone_surface}"
