...
--     09-Jul-2019 Helium+LUT-air for Peter Collen
config.title = "One-dimensional shock tube with air driving equil-air."
print(config.title)
config.dimensions = 3
...
print("before setGasModel")
nsp, nmodes = setGasModel('lut-plus-ideal-air-gas-model.lua')
...
function tube_gas(x, y, z)
   ...
   if x < 0.5 then
      -- Fill the left-half of the volume with high-pressure air.
      massFractions = {ideal=1.0, lut=0.0}
      p = 1.0e5; T = 348.4
   else
      -- and the right-half with low-pressure air.
      massFractions = {ideal=0.0, lut=1.0}
      p = 1.0e4; T = 278.8
   end
