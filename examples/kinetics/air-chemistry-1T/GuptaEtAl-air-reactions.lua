-- GuptaEtAl-air-reactions.lua
-- This chemical kinetic system provides a full 20 reaction scheme
-- for air with a single termodynamic temperature..
--
-- Reference:
-- ---------
...
-- The full 11-species are: N2, O2, N, O, NO, N+, O+, N2+, O2+, NO+, e-
-- A 7-species subset is  : N2, O2, N, O, NO, NO+, e-
-- A 5-species subset is  : N2, O2, N, O, NO
--
-- History
-- -------
-- 2016-01-19 : Updated by RJG for use with Dlang project
-- 2021-03-20 : Clean up subset selection (PJ).

SUBSET_SELECTION = '5-species-air'
-- Other options are '7-species-air', '11-species-air'.
-- The reaction subset selection is done at the end of this file.

-- We know that some of the species in the efficiencies list do NOT apply
-- when we don't consider ionization. We choose to suppress those warnings.
...
Config{
   ...
   -- This reaction scheme is often used in strongly-shocked flow.
   -- We err on the more expensive but (hopefully) more robust thermo coupling.
   --
   odeStep = {method='rkf', errTol=1.0e-5}
   -- For the same reason, we tighten the error tolerance.
}
...
if SUBSET_SELECTION == '5-species-air' then
   selectOnlyReactionsWithLabel({'r1', 'r2', 'r3', 'r4', 'r5', 'r6'})
end

if SUBSET_SELECTION == '7-species-air' then
   selectOnlyReactionsWithLabel({'r1', 'r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r14', 'r15'})
end
