...
# analytic_he_n2.py
#
# PJ, 2020-05-21, adapted from classic-shock-tube.py script.
...
#
print("shock-tube fill conditions with helium driving eq-n2")
gm_he = GasModel('ideal-helium-gas-model.lua')
gm_n2 = GasModel('cea-n2-gas-model.lua')
flow_n2 = GasFlow(gm_n2)
flow_he = GasFlow(gm_he)
states = []
states.append(GasState(gm_he))  # [0] to interpolate inside expansion
states.append(GasState(gm_n2))  # [1] driven
states.append(GasState(gm_n2))  # [2]
states.append(GasState(gm_he))  # [3]
states.append(GasState(gm_he))  # [4] driver
states[0].p = 1.35e6; states[0].T = 3000.0 # spare, to be used later
states[1].p = 500.0;  states[1].T = 700.0  # driven tube, initial
states[2].p = 500.0;  states[2].T = 700.0  # intermediate, post-shock
states[3].p = 1.35e6; states[3].T = 3000.0 # intermediate, post-expansion
states[4].p = 1.35e6; states[4].T = 3000.0 # driver tube, initial
...
def error_in_velocity(p3p4):
    ...
    v1s, v2, v2g = flow_n2.normal_shock_p2p1(states[1], p2/states[1].p, states[2])
    return (v3g - v2g)/v3g
#
...
v1s, v2, v2g = flow_n2.normal_shock_p2p1(states[1], p3/states[1].p, states[2])
